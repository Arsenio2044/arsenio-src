...
#define CREDITS_FILE_HL2 "scripts/credits.txt"
#define CREDITS_FILE_EP1 "scripts/ep1_credits.txt"
#define CREDITS_FILE_EP2 "scripts/ep2_credits.txt"
#define CREDITS_FILE_MOB "scripts/mob_credits.txt"
...
#define CREDITS_MOB_OUTRO 4
#define CREDITS_EP1_OUTRO 5
#define CREDITS_EP2_OUTRO 6
...
private:
	...
	void PrepareCredits( const char *pKeyName, const char *pCredFile );
	void DrawOutroCreditsName( void );
	void DrawIntroCreditsName( void );
	void DrawLogo( void );

	void PrepareLogo( float flTime );
	void PrepareOutroCredits( int nWhichCredits );
...
void CHudCredits::PrepareCredits( const char *pKeyName, const char *pCredFile )
{
	Clear();

	KeyValues *pKV= new KeyValues( "CreditsFile" );
	if (!pKV->LoadFromFile( filesystem, pCredFile, "MOD" ))
	{
		pKV->deleteThis();
...
void CHudCredits::DrawLogo( void )
{
	...
	{
		Q_snprintf( szLogoFont, sizeof( szLogoFont ), "Ep2ClientTitleFont" );
	}
	else
	{
...
void CHudCredits::PrepareLogo( float flTime )
{
	...
	PrepareCredits( NULL, CREDITS_FILE_HL2 );
...
void CHudCredits::PrepareOutroCredits( int nWhichCredits )
{
	const char* szCredFile;
	switch (nWhichCredits)
	{
	case CREDITS_OUTRO:
		szCredFile = CREDITS_FILE_HL2; break;
	case CREDITS_EP1_OUTRO:
		szCredFile = CREDITS_FILE_EP1; break;
	case CREDITS_EP2_OUTRO:
		szCredFile = CREDITS_FILE_EP2; break;
	case CREDITS_MOB_OUTRO:
		szCredFile = CREDITS_FILE_MOB; break;
	default:
		Msg( "Invalid credits :%d\n", nWhichCredits ); 
		return;
	}
	Msg( "Credits type %d\n", nWhichCredits );
...
{
	PrepareCredits( "IntroCreditsNames", CREDITS_FILE_HL2 );
...
void CHudCredits::MsgFunc_CreditsMsg( bf_read &msg )
{
	...
	switch ( m_iCreditsType )
	{
		...
		case CREDITS_OUTRO:     // fallthrough
		case CREDITS_MOB_OUTRO: // fallthrough
		case CREDITS_EP1_OUTRO: // fallthrough
		case CREDITS_EP2_OUTRO:
			PrepareOutroCredits( m_iCreditsType );
			break;
		default:
			Msg( "Unknown credits type: %d\n", m_iCreditsType );
	}
}

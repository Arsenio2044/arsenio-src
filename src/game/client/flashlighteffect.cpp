...
extern ConCommand Nvg_On; // CREDIT BREADMAN: - Probably don't need this.
...
#ifdef ARSENIO
static ConVar r_nvgfov("r_nvgfov", "100", FCVAR_CHEAT); // CREDIT BREADMAN: - Changed for NVG effect
#endif
...
CFlashlightEffect::~CFlashlightEffect()
{
	...
	if (IsNVG())
	{
		engine->ClientCmd("Nvg_On"); //CREDIT BREADMAN: - see View_scene.cpp for this Convar
	}
#endif
}
...
// Thse shouldn't actually function.
...
void CFlashlightEffect::UpdateLightOld(const Vector& vecPos, const Vector& vecDir, int nDistance)
{
	if (!m_pPointLight || (m_pPointLight->key != m_nEntIndex))
	{
		...
		if (IsNVG())
		{
			m_pPointLight->color.r = 5; // CREDIT BREADMAN: - Doesn't actually work here. Seems to be derived from the texture colour.
			m_pPointLight->color.g = 230; // CREDIT BREADMAN:
			m_pPointLight->color.b = 255; // CREDIT BREADMAN:
		}
#endif
	}

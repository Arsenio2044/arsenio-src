...
void CTriggerMultiple::ActivateMultiTrigger(CBaseEntity *pActivator)
{
	...
	if (Q_strcmp( GetEntityName().ToCStr(), "jeep_fall_trigger" ) == 0 &&
		Q_strcmp( gpGlobals->mapname.ToCStr(), "d2_coast_08") == 0)
	{
		Msg( "jeep_fall_trigger has been hacked not to fire at this point. Probably a good thing.\n" );
...
void CChangeLevel::Spawn( void )
{
	...
	/* MobMod HACK - was trying to make it so you could skip most of d1_eli_01, but
	   since you need Alyx to get out of the airlock in 02, there's no point
	if (Q_strcmp( STRING( gpGlobals->mapname ), "d1_eli_01" ) == 0 &&
		(Q_strcmp( STRING( GetEntityName() ), "changelevel_to_eli_02" ) == 0))
	{
		SetTouch( &CChangeLevel::TouchChangeLevel );
	}
	*/
...
void CChangeLevel::TouchChangeLevel( CBaseEntity *pOther )
{
	...
	//MobMod HACK to fix some f'dness when wallrunning through level transitions
	if ( pPlayer->m_nWallRunState > WALLRUN_NOT ) {
		
		// This is all from Gamemovement::EndWallRun()
		//Msg( "End Wallrun\n" );
		pPlayer->m_nWallRunState = WALLRUN_NOT;
		pPlayer->StopWallRunSound();


		pPlayer->m_nAirJumpState = AIRJUMP_NORM_JUMPING;
		pPlayer->DeriveMaxSpeed();

		Vector vecWallPush;
		VectorScale( pPlayer->m_vecWallNorm, 16.0f, vecWallPush );



		pPlayer->m_Local.m_vecTargetPunchAngle.Set( ROLL, 0 );
		pPlayer->SetEscapeVel( vec3_origin );
		pPlayer->m_flCoyoteTime = gpGlobals->curtime + sv_coyote_time.GetFloat();
	}
...
void CTriggerTeleport::Touch( CBaseEntity *pOther )
{
	...
	// Mobility Mod Hack to stop you getting stuck in Breen's office
	if (Q_strcmp( STRING( GetEntityName() ), "teleport_breen_blast_1" ) == 0)
	{
		Msg( "Disabling teleport trigger (workaround for problems with blackout.)\n" );
		Disable();
	}
...
void CTriggerCamera::Enable( void )
{
	...
	// HL2 blackout wakeup sequence doesn't work in ep2, so screw it 
	/*if (Q_strstr( STRING( gpGlobals->mapname ), "d1_trainstation_04" ) != NULL ||
		Q_strstr( STRING( gpGlobals->mapname ), "d3_breen_01" ) != NULL)
	{
		
		Msg( "point_viewcontroller not enabling because it's broken in this map.\n" );
		return;
	}*/

...
//
// Purpose: The Dark Crimson Mafia is a crime organisation.
//
//=============================================================================//

#include "cbase.h"

#include "npc_crim_s.h"
...
ConVar	arsenio_crims_health				( "arsenio_crims_health",					"45");
ConVar	arsenio_crims_heal_player			( "arsenio_crims_heal_player",				"25");
ConVar	arsenio_crims_heal_player_delay	( "arsenio_crims_heal_player_delay",		"25");
ConVar	arsenio_crims_giveammo_player_delay( "arsenio_crims_giveammo_player_delay",	"10");
ConVar	arsenio_crims_heal_player_min_pct	( "arsenio_crims_heal_player_min_pct",		"0.60");
ConVar	arsenio_crims_heal_player_min_forced( "arsenio_crims_heal_player_min_forced",		"10.0");
ConVar	arsenio_crims_heal_ally			( "arsenio_crims_heal_ally",				"30");
ConVar	arsenio_crims_heal_ally_delay		( "arsenio_crims_heal_ally_delay",			"20");
ConVar	arsenio_crims_heal_ally_min_pct	( "arsenio_crims_heal_ally_min_pct",		"0.90");
ConVar	arsenio_crims_player_stare_time	( "arsenio_crims_player_stare_time",		"1.0" );
ConVar  arsenio_crims_player_stare_dist	( "arsenio_crims_player_stare_dist",		"72" );
ConVar	arsenio_crims_stare_heal_time		( "arsenio_crims_stare_heal_time",			"5" );

ConVar	g_ai_crims_show_enemy( "g_ai_crims_show_enemy", "0" );

ConVar	npc_crim_s_insignia( "npc_crim_s_insignia", "0" );
ConVar	npc_crim_s_squad_marker( "npc_crim_s_squad_marker", "0" );
ConVar	npc_crim_s_explosive_resist( "npc_crim_s_explosive_resist", "0" );
ConVar	npc_crim_s_auto_player_squad( "npc_crim_s_auto_player_squad", "1" );
ConVar	npc_crim_s_auto_player_squad_allow_use( "npc_crim_s_auto_player_squad_allow_use", "0" );

ConVar	npc_crim_s_dont_precache_all( "npc_crim_s_dont_precache_all", "0" );


ConVar  npc_crim_s_medic_emit_sound("npc_crim_s_medic_emit_sound", "1" );
#ifdef HL2_EPISODIC
// todo: bake these into pound constants (for now they're not just for tuning purposes)
ConVar  npc_crim_s_heal_chuck_medkit("npc_crim_s_heal_chuck_medkit" , "1" , FCVAR_ARCHIVE, "Set to 1 to use new experimental healthkit-throwing medic.");
ConVar npc_crim_s_medic_throw_style( "npc_crim_s_medic_throw_style", "1", FCVAR_ARCHIVE, "Set to 0 for a lobbier trajectory" );
ConVar npc_crim_s_medic_throw_speed( "npc_crim_s_medic_throw_speed", "650" );
ConVar	arsenio_crims_heal_toss_player_delay("arsenio_crims_heal_toss_player_delay", "26", FCVAR_NONE, "how long between throwing healthkits" );


#define MEDIC_THROW_SPEED npc_crim_s_medic_throw_speed.GetFloat()
...
// Animation events
int AE_CRIMS_GET_PACKAGE;
int AE_CRIMS_HEAL;

//-------------------------------------
//-------------------------------------

ConVar	crims_follow_move_commands( "crims_follow_move_commands", "1" );
ConVar	ai_crims_debug_commander( "ai_crims_debug_commander", "1" );
#define DebuggingCommanderMode() (ai_crims_debug_commander.GetBool() && (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))

//-----------------------------------------------------------------------------
// Crims expressions for the citizen expression types
//-----------------------------------------------------------------------------
#define STATES_WITH_EXPRESSIONS		3		// Idle, Alert, Combat
#define EXPRESSIONS_PER_STATE		1

char *szExpressionTypeCrimss[CIT_EXP_LAST_TYPE] =
{
	"Unassigned",
	"Scared",
	"Normal",
...
struct crims_expression_list_t
{
	char *szExpressions[EXPRESSIONS_PER_STATE];
};
// Scared
crims_expression_list_t ScaredExpressions[STATES_WITH_EXPRESSIONS] =
{
...
// Normal
crims_expression_list_t NormalExpressions[STATES_WITH_EXPRESSIONS] =
{
...
// Angry
crims_expression_list_t AngryExpressions[STATES_WITH_EXPRESSIONS] =
{
...
int ACT_CRIMS_HANDSUP;
int	ACT_CRIMS_BLINDED;		// Blinded by scanner photo
int ACT_CRIMS_SHOWARMBAND;
int ACT_CRIMS_HEAL;
int	ACT_CRIMS_STARTLED;		// Startled by sneaky scanner

//---------------------------------------------------------

LINK_ENTITY_TO_CLASS( npc_crim_s, CNPC_CRIMS );

//---------------------------------------------------------

BEGIN_DATADESC( CNPC_CRIMS )
...
//						m_FuncTankBehavior
	...
	DEFINE_KEYFIELD(	m_ExpressionTypeCrims,			FIELD_INTEGER,	"expressiontype" ),
...
CSimpleSimTimer CNPC_CRIMS::gm_PlayerSquadEvaluateTimer;

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------

bool CNPC_CRIMS::CreateBehaviors()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::Precache()
{
	SelectModel();
	SelectExpressionTypeCrims();

	if ( !npc_crim_s_dont_precache_all.GetBool() )
	...
	PrecacheScriptSound( "NPC_Crims.FootstepLeft" );
	PrecacheScriptSound( "NPC_Crims.FootstepRight" );
	PrecacheScriptSound( "NPC_Crims.Die" );

	PrecacheInstancedScene( "scenes/Expressions/CrimsIdle.vcd" );
	PrecacheInstancedScene( "scenes/Expressions/CrimsAlert_loop.vcd" );
	PrecacheInstancedScene( "scenes/Expressions/CrimsCombat_loop.vcd" );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::PrecacheAllOfType( CrimsType_t type )
{
	if ( m_Type == CT_UNIQUE )
		return;
...
void CNPC_CRIMS::Spawn()
{
	...
	m_iHealth = arsenio_crims_health.GetFloat();
	...
	SetUse( &CNPC_CRIMS::CommanderUse );
...
void CNPC_CRIMS::SelectModel()
{
	...
	{
		struct CrimsTypeMapping
		{
			const char *pszMapTag;
			CrimsType_t type;
		};

		static CrimsTypeMapping CrimsTypeMappings[] = 
		{
		...
		for ( int i = 0; i < ARRAYSIZE(CrimsTypeMappings); i++ )
		{
			if ( Q_stristr( szMapName, CrimsTypeMappings[i].pszMapTag ) )
			{
				m_Type = CrimsTypeMappings[i].type;
				break;
			}
		}
	...
	if( HasSpawnFlags( SF_CITIZEN_RANDOM_HEAD | SF_CITIZEN_RANDOM_HEAD_MALE | SF_CITIZEN_RANDOM_HEAD_FEMALE ) || GetModelName() == NULL_STRING )
	{
		...
		{
			SetModelName( AllocPooledString("models/crim/male_cheaple.mdl" ) );
			return;
		}
		else
		{
			...
			{
				CNPC_CRIMS *pCrims = dynamic_cast<CNPC_CRIMS *>(g_AI_Manager.AccessAIs()[i]);
				if ( pCrims && pCrims != this && pCrims->m_iHead >= 0 && pCrims->m_iHead < ARRAYSIZE(g_ppszRandomHeads) )
				{
					headCounts[pCrims->m_iHead]++;
				}
			}
	...
	if ( m_Type != CT_UNIQUE )
	{
		SetModelName( AllocPooledString( CFmtStr( "models/crim/%s/%s", (const char *)(CFmtStr(g_ppszModelLocs[ m_Type ], ( IsMedic() ) ? "m" : "" )), pszModelName ) ) );
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CRIMS::SelectExpressionTypeCrims()
{
	// If we've got a mapmaker assigned type, leave it alone
	if ( m_ExpressionTypeCrims != CIT_EXP_UNASSIGNED )
		return;

	switch ( m_Type )
	{
	case CT_DOWNTRODDEN:
		m_ExpressionTypeCrims = (CrimsExpressionTypeCrimss_t)RandomInt( CIT_EXP_SCARED, CIT_EXP_NORMAL );
		break;
	case CT_REFUGEE:
		m_ExpressionTypeCrims = (CrimsExpressionTypeCrimss_t)RandomInt( CIT_EXP_SCARED, CIT_EXP_NORMAL );
		break;
	case CT_REBEL:
		m_ExpressionTypeCrims = (CrimsExpressionTypeCrimss_t)RandomInt( CIT_EXP_SCARED, CIT_EXP_ANGRY );
		break;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::FixupMattWeapon()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
string_t CNPC_CRIMS::GetModelName() const
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::ShouldAlwaysThink() 
{ 
...
bool CNPC_CRIMS::ShouldBehaviorSelectSchedule( CAI_BehaviorBase *pBehavior )
{
...
void CNPC_CRIMS::OnChangeRunningBehavior( CAI_BehaviorBase *pOldBehavior,  CAI_BehaviorBase *pNewBehavior )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::GatherConditions()
{
	...
	if ( !SpokeConcept( TLK_JOINPLAYER ) && IsRunningScriptedSceneWithSpeech( this, true ) )
	{
		...
		for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			...
			{
				(assert_cast<CNPC_CRIMS *>(pNpc))->SetSpokeConcept( TLK_JOINPLAYER, NULL );
			}
		}
	}
	...
	if( CanHeal() )
	{
		...
		float flStareDist = arsenio_crims_player_stare_dist.GetFloat();
		...
		if( pPlayer->IsAlive() && flPlayerDamage > 0 && (flDistSqr <= flStareDist * flStareDist) && pPlayer->FInViewCone( this ) && pPlayer->FVisible( this ) )
		{
			...
			if( gpGlobals->curtime - m_flTimePlayerStare >= arsenio_crims_player_stare_time.GetFloat() && gpGlobals->curtime > m_flTimeNextHealStare && !IsCurSchedule( SCHED_CITIZEN_HEAL ) )
			{
				...
				{
					m_flTimeNextHealStare = gpGlobals->curtime + arsenio_crims_stare_heal_time.GetFloat() * .5f;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::PredictPlayerPush()
{
	if ( !AI_IsSinglePlayer() )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::PrescheduleThink()
{
	...
	if ( !npc_crim_s_insignia.GetBool() && npc_crim_s_squad_marker.GetBool() && IsInPlayerSquad() )
	{
	...
	if( GetEnemy() && g_ai_crims_show_enemy.GetBool() )
	{
...
//-----------------------------------------------------------------------------
void CNPC_CRIMS::BuildScheduleTestBits()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::FInViewCone( CBaseEntity *pEntity )
{
#if 0
	if ( IsMortar( pEntity ) )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_CRIMS::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
...
int CNPC_CRIMS::SelectSchedule()
{
	...
	{
		DevMsg( "Crims in select schedule but RPG is guiding?\n");
		pRPG->StopGuiding();
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_CRIMS::SelectSchedulePriorityAction()
{
...
//-----------------------------------------------------------------------------
int CNPC_CRIMS::SelectScheduleHeal()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_CRIMS::SelectScheduleRetrieveItem()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_CRIMS::SelectScheduleNonCombat()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_CRIMS::SelectScheduleManhackCombat()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_CRIMS::SelectScheduleCombat()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::ShouldDeferToFollowBehavior()
{
#if 0
	if ( HaveCommandGoal() )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_CRIMS::TranslateSchedule( int scheduleType ) 
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::ShouldAcceptGoal( CAI_BehaviorBase *pBehavior, CAI_GoalEntity *pGoal )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::OnClearGoal( CAI_BehaviorBase *pBehavior, CAI_GoalEntity *pGoal )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	...
	case TASK_CIT_HEAL_TOSS:
		...
		SetIdealActivity( (Activity)ACT_CRIMS_HEAL );
		break;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
...
// Input  : code - 
//-----------------------------------------------------------------------------
void CNPC_CRIMS::TaskFail( AI_TaskFailureCode_t code )
{
	// If our heal task has failed, push out the heal time
	if ( IsCurSchedule( SCHED_CITIZEN_HEAL ) )
	{
		m_flPlayerHealTime 	= gpGlobals->curtime + arsenio_crims_heal_ally_delay.GetFloat();
	}
...
//-----------------------------------------------------------------------------
Activity CNPC_CRIMS::NPC_TranslateActivity( Activity activity )
{
	...
	// !!!HACK - Crimss don't have the required animations for shotguns, 
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_CRIMS::HandleAnimEvent( animevent_t *pEvent )
{
	if ( pEvent->event == AE_CRIMS_GET_PACKAGE )
	{
	...
	else if ( pEvent->event == AE_CRIMS_HEAL )
	{
		...
		if ( USE_EXPERIMENTAL_MEDIC_CODE() && IsMedic() )
		{
			...
			if ( pTarget )
			{
				m_flPlayerHealTime 	= gpGlobals->curtime + arsenio_crims_heal_toss_player_delay.GetFloat();;
	...
	case NPC_EVENT_LEFTFOOT:
		{
			EmitSound( "NPC_Crims.FootstepLeft", pEvent->eventtime );
		}
		break;

	case NPC_EVENT_RIGHTFOOT:
		{
			EmitSound( "NPC_Crims.FootstepRight", pEvent->eventtime );
		}
		break;
...
#ifdef ARSENIO
void CNPC_CRIMS::Event_Killed(const CTakeDamageInfo& info)
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_CRIMS::PickupItem( CBaseEntity *pItem )
{
	Assert( pItem != NULL );
	...
	{
		DevMsg("Crims doesn't know how to pick up %s!\n", pItem->GetClassname() );
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::IgnorePlayerPushing( void )
{
...
//-----------------------------------------------------------------------------
const char *CNPC_CRIMS::SelectRandomExpressionForState( NPC_STATE state )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::SimpleUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::OnBeginMoveAndShoot()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::OnEndMoveAndShoot()
{
	VacateStrategySlot();
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::LocateEnemySound()
{
#if 0
	if ( !GetEnemy() )
		return;
	...
	if( flZDiff < -128 )
	{
		EmitSound( "NPC_Crims.UpThere" );
	}
	else if( flZDiff > 128 )
	{
		EmitSound( "NPC_Crims.DownThere" );
	}
	else
	{
		EmitSound( "NPC_Crims.OverHere" );
	}
#endif
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::IsManhackMeleeCombatant()
{
	CBaseCombatWeapon *pWeapon = GetActiveWeapon();
...
//-----------------------------------------------------------------------------
Vector CNPC_CRIMS::GetActualShootPosition( const Vector &shootOrigin )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::OnChangeActiveWeapon( CBaseCombatWeapon *pOldWeapon, CBaseCombatWeapon *pNewWeapon )
{
	if ( pNewWeapon )
	{
...
bool CNPC_CRIMS::ShouldLookForBetterWeapon()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_CRIMS::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
#define CITIZEN_SCORCH_FLOOR	75
	...
	{
		if( npc_crim_s_explosive_resist.GetBool() )
		{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::IsPlayerAlly( CBasePlayer *pPlayer )											
{ 
	if ( Classify() == CLASS_CITIZEN_PASSIVE && GlobalEntity_GetState("gordon_precriminal") == GLOBAL_ON )
	{
		// Robin: Crimss use friendly speech semaphore in trainstation
		return true;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::CanJoinPlayerSquad()
{
	if ( !AI_IsSinglePlayer() )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::WasInPlayerSquad()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::HaveCommandGoal() const			
{	
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::IsCommandMoving()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::ShouldAutoSummon()
{
...
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::IsValidCommandTarget( CBaseEntity *pTarget )
{
	return false;
}

//-----------------------------------------------------------------------------
bool CNPC_CRIMS::SpeakCommandResponse( AIConcept_t concept, const char *modifiers )
{
...
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::TargetOrder( CBaseEntity *pTarget, CAI_BaseNPC **Allies, int numAllies )
{
	if ( pTarget->IsPlayer() )
	{
...
//-----------------------------------------------------------------------------
void CNPC_CRIMS::MoveOrder( const Vector &vecDest, CAI_BaseNPC **Allies, int numAllies )
{
	if ( !AI_IsSinglePlayer() )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::CommanderUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pActivator == UTIL_GetLocalPlayer() )
	{
		...
		if ( npc_crim_s_auto_player_squad_allow_use.GetBool() )
		{
			if ( !ShouldAutosquad() )
				TogglePlayerSquadState();
			else if ( !IsInPlayerSquad() && npc_crim_s_auto_player_squad_allow_use.GetBool() )
				AddToPlayerSquad();
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::ShouldSpeakRadio( CBaseEntity *pListener )
{
	if ( !pListener )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::OnMoveToCommandGoalFailed()
{
	// Clear the goal.
	SetCommandGoal( vec3_invalid );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::AddToPlayerSquad()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::RemoveFromPlayerSquad()
{
	Assert( IsInPlayerSquad() );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::TogglePlayerSquadState()
{
	if ( !AI_IsSinglePlayer() )
		return;
...
void CNPC_CRIMS::UpdatePlayerSquad()
{
	if ( !AI_IsSinglePlayer() )
		return;
	...
	if ( pPlayerSquad )
	{
		CUtlVectorFixed<CNPC_CRIMS *, MAX_PLAYER_SQUAD> squadMembersToRemove;
		AISquadIter_t iter;
		...
		for ( CAI_BaseNPC *pPlayerSquadMember = pPlayerSquad->GetFirstMember(&iter); pPlayerSquadMember; pPlayerSquadMember = pPlayerSquad->GetNextMember(&iter) )
		{
			...
			CNPC_CRIMS *pCrims = assert_cast<CNPC_CRIMS *>(pPlayerSquadMember);

			if ( !pCrims->m_bNeverLeavePlayerSquad &&
				 pCrims->m_FollowBehavior.GetFollowTarget() &&
				 !pCrims->m_FollowBehavior.FollowTargetVisible() && 
				 pCrims->m_FollowBehavior.GetNumFailedFollowAttempts() > 0 && 
				 gpGlobals->curtime - pCrims->m_FollowBehavior.GetTimeFailFollowStarted() > 20 &&
				 ( fabsf(( pCrims->m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().z - pCrims->GetAbsOrigin().z )) > 196 ||
				   ( pCrims->m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().AsVector2D() - pCrims->GetAbsOrigin().AsVector2D() ).LengthSqr() > Square(50*12) ) )
			{
				if ( DebuggingCommanderMode() )
				{
					DevMsg( "Player follower is lost (%d, %f, %d)\n", 
						 pCrims->m_FollowBehavior.GetNumFailedFollowAttempts(), 
						 gpGlobals->curtime - pCrims->m_FollowBehavior.GetTimeFailFollowStarted(), 
						 (int)((pCrims->m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().AsVector2D() - pCrims->GetAbsOrigin().AsVector2D() ).Length()) );
				}

				squadMembersToRemove.AddToTail( pCrims );
			}
		}
	...
	if ( pPlayer && ShouldAutosquad() && !(pPlayer->GetFlags() & FL_NOTARGET ) && pPlayer->IsAlive() )
	{
		...
		for ( i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			...
			CNPC_CRIMS *pCrims = assert_cast<CNPC_CRIMS *>(ppAIs[i]);
			int iNew;

			if ( pCrims->IsInPlayerSquad() )
			{
				iNew = candidates.AddToTail();
				candidates[iNew].pCrims = pCrims;
				candidates[iNew].bIsInSquad = true;
				candidates[iNew].distSq = 0;
				candidates[iNew].iSquadIndex = pCrims->GetSquad()->GetSquadIndex( pCrims );
			}
			else
			{
				float distSq = (vPlayerPos.AsVector2D() - pCrims->GetAbsOrigin().AsVector2D()).LengthSqr(); 
				if ( distSq > JOIN_PLAYER_XY_TOLERANCE_SQ && 
					( pCrims->m_flTimeJoinedPlayerSquad == 0 || gpGlobals->curtime - pCrims->m_flTimeJoinedPlayerSquad > 60.0 ) && 
					( pCrims->m_flTimeLastCloseToPlayer == 0 || gpGlobals->curtime - pCrims->m_flTimeLastCloseToPlayer > 15.0 ) )
					continue;

				if ( !pCrims->CanJoinPlayerSquad() )
					continue;

				bool bShouldAdd = false;

				if ( pCrims->HasCondition( COND_SEE_PLAYER ) )
					bShouldAdd = true;
				else
				{
					bool bPlayerVisible = pCrims->FVisible( pPlayer );
					if ( bPlayerVisible )
					{
						if ( pCrims->HasCondition( COND_HEAR_PLAYER ) )
							bShouldAdd = true;
						else if ( distSq < UNCONDITIONAL_JOIN_PLAYER_XY_TOLERANCE_SQ && fabsf(vPlayerPos.z - pCrims->GetAbsOrigin().z) < UNCONDITIONAL_JOIN_PLAYER_Z_TOLERANCE )
							bShouldAdd = true;
					}
				}
				...
				if ( bShouldAdd )
				{
					...
					AI_Waypoint_t *pPathToPlayer = pCrims->GetPathfinder()->BuildRoute( pCrims->GetAbsOrigin(), vPlayerPos, pPlayer, 5*12, NAV_NONE, true );
		...
		if ( bFoundNewGuy )
		{
			...
			for ( i = 0; i < initialCount; i++ )
				candidates[i].pCrims->AddSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE ); // Prevents double-add
			for ( i = 0; i < initialCount; i++ )
			{
				if ( candidates[i].iSquadIndex == -1 )
				{
					for ( int j = 0; j < g_AI_Manager.NumAIs(); j++ )
					{
						...
						CNPC_CRIMS *pCrims = assert_cast<CNPC_CRIMS *>(ppAIs[j]);

						float distSq = (vPlayerPos - pCrims->GetAbsOrigin()).Length2DSqr(); 
						if ( distSq > JOIN_PLAYER_XY_TOLERANCE_SQ )
							continue;

						distSq = (candidates[i].pCrims->GetAbsOrigin() - pCrims->GetAbsOrigin()).Length2DSqr(); 
						...
						candidates[iNew].distSq = distSq;
						candidates[iNew].iSquadIndex = -1;
						pCrims->AddSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE ); // Prevents double-add
					}
				}
			}
			for ( i = 0; i < candidates.Count(); i++ )
				candidates[i].pCrims->RemoveSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE );
			...
			if ( candidates.Count() > MAX_PLAYER_SQUAD )
			{
				...
				{
					if ( candidates[i].pCrims->IsInPlayerSquad() )
					{
						candidates[i].pCrims->RemoveFromPlayerSquad();
					}
				}
			}
			...
			if ( candidates.Count() )
			{
				...
				for ( i = 0; i < candidates.Count() && i < MAX_PLAYER_SQUAD; i++ )
				{
					if ( !candidates[i].pCrims->IsInPlayerSquad() )
					{
						candidates[i].pCrims->AddToPlayerSquad();
						nJoined++;

						if ( candidates[i].distSq < closestDistSq )
						{
							pClosest = candidates[i].pCrims;
				...
				if ( pClosest )
				{
					...
					for ( i = 0; i < candidates.Count() && i < MAX_PLAYER_SQUAD; i++ )
					{
						candidates[i].pCrims->SetSpokeConcept( TLK_JOINPLAYER, NULL ); 
					}
				}
			}
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_CRIMS::PlayerSquadCandidateSortFunc( const SquadCandidate_t *pLeft, const SquadCandidate_t *pRight )
{
	// "Bigger" means less approprate 
	CNPC_CRIMS *pLeftCrims = pLeft->pCrims;
	CNPC_CRIMS *pRightCrims = pRight->pCrims;

	// Medics are better than anyone
	if ( pLeftCrims->IsMedic() && !pRightCrims->IsMedic() )
		return -1;

	if ( !pLeftCrims->IsMedic() && pRightCrims->IsMedic() )
		return 1;

	CBaseCombatWeapon *pLeftWeapon = pLeftCrims->GetActiveWeapon();
	CBaseCombatWeapon *pRightWeapon = pRightCrims->GetActiveWeapon();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::FixupPlayerSquad()
{
	if ( !AI_IsSinglePlayer() )
		return;
	...
	if ( m_pSquad->NumMembers() > MAX_PLAYER_SQUAD )
	{
		...
		CNPC_CRIMS *pFirstMemberCrims = dynamic_cast< CNPC_CRIMS * >( pFirstMember );
		if ( pFirstMemberCrims )
		{
			pFirstMemberCrims->ClearFollowTarget();
		}
		else
		{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::ClearFollowTarget()
{
	m_FollowBehavior.SetFollowTarget( NULL );
	m_FollowBehavior.SetParameters( AIF_SIMPLE );
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::UpdateFollowCommandPoint()
{
	if ( !AI_IsSinglePlayer() )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::IsFollowingCommandPoint()
{
	CBaseEntity *pFollowTarget = m_FollowBehavior.GetFollowTarget();
	if ( pFollowTarget )
...
CAI_BaseNPC *CNPC_CRIMS::GetSquadCommandRepresentative()
{
	if ( !AI_IsSinglePlayer() )
		return NULL;
	...
	if ( IsInPlayerSquad() )
	{
		...
		if ( gpGlobals->curtime - lastTime > 2.0 || !hCurrent || !hCurrent->IsInPlayerSquad() ) // hCurrent will be NULL after level change
		{
			...
			if ( pPlayer )
			{
				...
				for ( CAI_BaseNPC *pAllyNpc = m_pSquad->GetFirstMember(&iter); pAllyNpc; pAllyNpc = m_pSquad->GetNextMember(&iter) )
				{
					if ( pAllyNpc->IsCommandable() && dynamic_cast<CNPC_CRIMS *>(pAllyNpc) )
					{
						int i = candidates.AddToTail();
						candidates[i].pMember = (CNPC_CRIMS *)(pAllyNpc);
		...
		if ( hCurrent != NULL )
		{
			Assert( dynamic_cast<CNPC_CRIMS *>(hCurrent.Get()) && hCurrent->IsInPlayerSquad() );
			return hCurrent;
		}
	}
	return NULL;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::SetSquad( CAI_Squad *pSquad )
{
	...
	if( IsInPlayerSquad() && !bWasInPlayerSquad )
	{
		...
		if ( npc_crim_s_insignia.GetBool() )
			AddInsignia();
	}
...
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	...
	else if (interactionType ==	g_interactionScannerInspectBegin)
	{
		...
		{
			m_nInspectActivity = ACT_CRIMS_STARTLED;
		}
		else
		{
			// Otherwise we're blinded by the flash
			m_nInspectActivity = ACT_CRIMS_BLINDED;
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::FValidateHintType( CAI_Hint *pHint )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::ShouldHealTarget( CBaseEntity *pTarget, bool bActiveUse )
{
			...
			)
	 	{
			if ( pTarget->m_iHealth > 0 )
			{
	 			if ( bActiveUse )
				{
					...
					float timeRecharge = arsenio_crims_heal_player_delay.GetFloat();
					float maximumHealAmount = arsenio_crims_heal_player.GetFloat();
					...
					if ( healAmt < arsenio_crims_heal_player_min_forced.GetFloat() )
						return false;
				...
				if ( bReadyToHeal )
				{
					...
					if ( bTargetIsPlayer )
						requiredHealth = pTarget->GetMaxHealth() - arsenio_crims_heal_player.GetFloat();
					else
						requiredHealth = pTarget->GetMaxHealth() * arsenio_crims_heal_player_min_pct.GetFloat();
	...
	if ( IsAmmoResupplier() && bTargetIsPlayer )
	{
		if ( m_flPlayerGiveAmmoTime <= gpGlobals->curtime )
		{
			...
			if ( iAmmoType == -1 )
			{
				DevMsg("ERROR: Crims attempting to give unknown ammo type (%s)\n", STRING(m_iszAmmoSupply) );
			}
			else
			{
...
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::ShouldHealTossTarget( CBaseEntity *pTarget, bool bActiveUse )
{
	Disposition_t disposition;
...
void CNPC_CRIMS::Heal()
{
	...
	m_flTimeNextHealStare = gpGlobals->curtime + arsenio_crims_stare_heal_time.GetFloat();
	...
	if ( IsMedic() )
	{
		...
		if ( pTarget->IsPlayer() )
		{
			...
			timeRecharge 		= arsenio_crims_heal_player_delay.GetFloat();
			maximumHealAmount 	= arsenio_crims_heal_player.GetFloat();
			m_flPlayerHealTime 	= gpGlobals->curtime + timeRecharge;
		}
		else
		{
			timeFullHeal 		= m_flAllyHealTime;
			timeRecharge 		= arsenio_crims_heal_ally_delay.GetFloat();
			maximumHealAmount 	= arsenio_crims_heal_ally.GetFloat();
		...
		if ( healAmt > 0 )
		{
			if ( pTarget->IsPlayer() && npc_crim_s_medic_emit_sound.GetBool() )
			{
	...
	if ( IsAmmoResupplier() )
	{
		...
		if ( pTarget->IsPlayer() )
		{
			...
			m_flPlayerGiveAmmoTime = gpGlobals->curtime + arsenio_crims_giveammo_player_delay.GetFloat();
		}
	}
}
...
//-----------------------------------------------------------------------------
void	CNPC_CRIMS::TossHealthKit(CBaseCombatCharacter *pThrowAt, const Vector &offset)
{
	Assert( pThrowAt );
	...
	if (npc_crim_s_medic_throw_style.GetInt() == 0)
	{
	...
	{
		Warning("Crims tried to heal but could not spawn item_healthkit!\n");
	}
...
//-----------------------------------------------------------------------------
void	CNPC_CRIMS::InputForceHealthKitToss( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::ShouldLookForHealthItem()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::InputStartPatrolling( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::InputStopPatrolling( inputdata_t &inputdata )
{
	m_bShouldPatrol = false;
}
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_CRIMS::OnGivenWeapon( CBaseCombatWeapon *pNewWeapon )
{
	FixupMattWeapon();
}
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_CRIMS::InputSetCommandable( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_CRIMS::InputSetMedicOn( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_CRIMS::InputSetMedicOff( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_CRIMS::InputSetAmmoResupplierOn( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_CRIMS::InputSetAmmoResupplierOff( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_CRIMS::InputSpeakIdleResponse( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CRIMS::DeathSound( const CTakeDamageInfo &info )
{
	// Sentences don't play on dead NPCs
	SentenceStop();

	EmitSound( "NPC_Crims.Die" );
}

//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_CRIMS::FearSound( void )
{
}
...
//-----------------------------------------------------------------------------
bool CNPC_CRIMS::UseSemaphore( void )
{
...
AI_BEGIN_CUSTOM_NPC( npc_crim_s, CNPC_CRIMS )
	...
	DECLARE_ACTIVITY( ACT_CRIMS_HANDSUP )
	DECLARE_ACTIVITY( ACT_CRIMS_BLINDED )
	DECLARE_ACTIVITY( ACT_CRIMS_SHOWARMBAND )
	DECLARE_ACTIVITY( ACT_CRIMS_HEAL )
	DECLARE_ACTIVITY( ACT_CRIMS_STARTLED )
	...
	//Events
	DECLARE_ANIMEVENT( AE_CRIMS_GET_PACKAGE )
	DECLARE_ANIMEVENT( AE_CRIMS_HEAL )
...
//
//==================================================================================================================
CHandle<CCrimsResponseSystem>	g_pCrimsResponseSystem = NULL;

CCrimsResponseSystem	*GetCrimsResponse()
{
	return g_pCrimsResponseSystem;
}

char *CrimsResponseConcepts[MAX_CITIZEN_RESPONSES] = 
{
...
LINK_ENTITY_TO_CLASS( ai_crims_response_system, CCrimsResponseSystem );

BEGIN_DATADESC( CCrimsResponseSystem )
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCrimsResponseSystem::Spawn()
{
	if ( g_pCrimsResponseSystem )
	{
		Warning("Multiple citizen response systems in level.\n");
		UTIL_Remove( this );
		return;
	}
	g_pCrimsResponseSystem = this;

	// Invisible, non solid.
	AddSolidFlags( FSOLID_NOT_SOLID );
	AddEffects( EF_NODRAW );
	SetThink( &CCrimsResponseSystem::ResponseThink );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCrimsResponseSystem::OnRestore()
{
	BaseClass::OnRestore();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCrimsResponseSystem::AddResponseTrigger( crimsresponses_t	iTrigger )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCrimsResponseSystem::InputResponseVitalNPC( inputdata_t &inputdata )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCrimsResponseSystem::ResponseThink()
{
		...
		for ( int i = 0; i < MAX_CITIZEN_RESPONSES; i++ )
		{
			if ( m_flResponseAddedTime[i] )
			{
				...
				else if ( m_flNextResponseTime < gpGlobals->curtime )
				{
					...
					CBaseEntity *pNearestCrims = NULL;
					CBaseEntity *pCrims = NULL;
					CBasePlayer *pPlayer = UTIL_GetLocalPlayer();
					while ( (pCrims = gEntList.FindEntityByClassname( pCrims, "npc_crim_s" ) ) != NULL)
					{
						float flDistToPlayer = (pPlayer->WorldSpaceCenter() - pCrims->WorldSpaceCenter()).LengthSqr();
						if ( flDistToPlayer < flNearestDist )
						{
							flNearestDist = flDistToPlayer;
							pNearestCrims = pCrims;
						}
					}

					// Found one?
					if ( pNearestCrims && ((CNPC_CRIMS*)pNearestCrims)->RespondedTo( CrimsResponseConcepts[i], false, false ) )
					{
						m_flResponseAddedTime[i] = 0;
...
void CNPC_CRIMS::RemoveInsignia()
{
...
//-----------------------------------------------------------------------------
int CNPC_CRIMS::DrawDebugTextOverlays( void ) 
{
	...
	if (m_debugOverlays & OVERLAY_TEXT_BIT) 
	{
		...
		Q_snprintf(tempstr,sizeof(tempstr),"Expression type: %s", szExpressionTypeCrimss[m_ExpressionTypeCrims]);

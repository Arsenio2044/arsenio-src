...
ConVar arsenio_walkspeed( "arsenio_walkspeed", "0" );
ConVar arsenio_normspeed( "arsenio_normspeed", "190" );
ConVar arsenio_sprintspeed( "arsenio_sprintspeed", "250" );
ConVar arsenio_incapspeed( "arsenio_incapspeed", "150" );


ConVar arsenio_darkness_flashlight_factor ( "arsenio_darkness_flashlight_factor", "1" );
...
#else
	#define	HL2_WALK_SPEED arsenio_walkspeed.GetFloat()
	#define	HL2_NORM_SPEED arsenio_normspeed.GetFloat()
	#define	HL2_SPRINT_SPEED arsenio_sprintspeed.GetFloat()
	#define	HL2_INCAP_SPEED arsenio_incapspeed.GetFloat()
...
bool Flashlight_UseLegacyVersion( void )
{
	...
	if ( g_bCacheLegacyFlashlightStatus )
	{
		...
		g_bUseLegacyFlashlight = ( !Q_strcmp( modDir, "arsenio" ) ||
...
int CHL2_Player::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if( (info.GetDamageType() & DMG_SLASH) && arsenio_episodic.GetBool() )
	{
...
void CLogicPlayerProxy::InputSetFlashlightSlowDrain( inputdata_t &inputdata )
{
	...
	if( pPlayer )
		pPlayer->SetFlashlightPowerDrainScale( arsenio_darkness_flashlight_factor.GetFloat() );
}

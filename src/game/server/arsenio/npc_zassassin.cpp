...
DEFINE_FIELD(CNPC_ZombieAssassin, m_nZombieAssassinSpitSprite, FIELD_INTEGER),
DEFINE_FIELD(CNPC_ZombieAssassin, m_hSprite, FIELD_EHANDLE),
...
void CNPC_ZombieAssassin::HandleAnimEvent(animevent_t* pEvent)
{
	...
	case ZOMBIE_ASSASSIN_AE_SPIT:
	{
		if (GetEnemy())
		{
			...
			CGrenadeSpit* pGrenade = (CGrenadeSpit*)CreateNoSpawn("grenade_spit", vecSpitDir, vec3_angle, this);
			pGrenade->Spawn();

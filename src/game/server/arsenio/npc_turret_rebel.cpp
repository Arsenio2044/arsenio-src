...
float CNPC_RebelTurret::fMaxTipControllerVelocity = 300.0f * 300.0f;
float CNPC_RebelTurret::fMaxTipControllerAngularVelocity = 90.0f * 90.0f;

#define	LASER_BEAM_SPRITE			"effects/laser1.vmt"

#define	REBEL_TURRET_MODEL			"models/combine_turrets/floor_turret.mdl"
#define	REBEL_TURRET_MODEL_CITIZEN	"models/combine_turrets/citizen_turret.mdl"
#define REBEL_TURRET_GLOW_SPRITE	"sprites/glow1.vmt"
// #define REBEL_TURRET_BC_YAW			"aim_yaw"
// #define REBEL_TURRET_BC_PITCH		"aim_pitch"
#define	REBEL_TURRET_RANGE			1200
#define	REBEL_TURRET_MAX_WAIT		5
#define REBEL_TURRET_SHORT_WAIT		2.0		// Used for FAST_RETIRE spawnflag
#define	REBEL_TURRET_PING_TIME		1.0f	//LPB!!

#define	REBEL_TURRET_VOICE_PITCH_LOW	45
#define	REBEL_TURRET_VOICE_PITCH_HIGH	100

//Aiming variables
#define	REBEL_TURRET_MAX_NOHARM_PERIOD		0.0f
#define	REBEL_TURRET_MAX_GRACE_PERIOD		3.0f

//Activities
int ACT_REBEL_TURRET_OPEN;
int ACT_REBEL_TURRET_CLOSE;
int ACT_REBEL_TURRET_OPEN_IDLE;
int ACT_REBEL_TURRET_CLOSED_IDLE;
int ACT_REBEL_TURRET_FIRE;

//Datatable
BEGIN_DATADESC( CNPC_RebelTurret )
...
LINK_ENTITY_TO_CLASS( npc_turret_floor, CNPC_RebelTurret );

//-----------------------------------------------------------------------------
// Constructor
//-----------------------------------------------------------------------------
CNPC_RebelTurret::CNPC_RebelTurret( void ) : 
	m_bActive( false ),
	m_hEyeGlow( NULL ),
	m_hLaser( NULL ),
	m_iAmmoType( -1 ),
...
// Purpose: 
//-----------------------------------------------------------------------------
Class_T	CNPC_RebelTurret::Classify( void ) 
{
	if ( m_bEnabled ) 
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::UpdateOnRemove( void )
{
...
// Purpose: Precache
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::Precache( void )
{
	const char *pModelName = STRING( GetModelName() );
	pModelName = ( pModelName && pModelName[ 0 ] != '\0' ) ? pModelName : REBEL_TURRET_MODEL;
	PrecacheModel( pModelName );
	PrecacheModel( REBEL_TURRET_GLOW_SPRITE );
	...
	if ( IsCitizenTurret() )
	{
		...
		PrecacheScriptSound( "NPC_RebelTurret.AlarmPing");
	}

	// Activities
	ADD_CUSTOM_ACTIVITY( CNPC_RebelTurret, ACT_REBEL_TURRET_OPEN );
	ADD_CUSTOM_ACTIVITY( CNPC_RebelTurret, ACT_REBEL_TURRET_CLOSE );
	ADD_CUSTOM_ACTIVITY( CNPC_RebelTurret, ACT_REBEL_TURRET_CLOSED_IDLE );
	ADD_CUSTOM_ACTIVITY( CNPC_RebelTurret, ACT_REBEL_TURRET_OPEN_IDLE );
	ADD_CUSTOM_ACTIVITY( CNPC_RebelTurret, ACT_REBEL_TURRET_FIRE );
	
	PrecacheScriptSound( "NPC_RebelTurret.Retire" );
	PrecacheScriptSound( "NPC_RebelTurret.Deploy" );
	PrecacheScriptSound( "NPC_RebelTurret.Move" );
	PrecacheScriptSound( "NPC_Combine.WeaponBash" );
	PrecacheScriptSound( "NPC_RebelTurret.Activate" );
	PrecacheScriptSound( "NPC_RebelTurret.Alert" );
	m_ShotSounds = PrecacheScriptSound( "NPC_RebelTurret.ShotSounds" );
	PrecacheScriptSound( "NPC_RebelTurret.Die" );
	PrecacheScriptSound( "NPC_RebelTurret.Retract");
	PrecacheScriptSound( "NPC_RebelTurret.Alarm");
	PrecacheScriptSound( "NPC_RebelTurret.Ping");
	PrecacheScriptSound( "NPC_RebelTurret.DryFire");
	PrecacheScriptSound( "NPC_RebelTurret.Destruct" );
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::Spawn( void )
{ 
	Precache();

	const char *pModelName = STRING( GetModelName() );
	SetModel( ( pModelName && pModelName[ 0 ] != '\0' ) ? pModelName : REBEL_TURRET_MODEL );
	...
	m_spawnflags |= SF_REBEL_TURRET_AUTOACTIVATE;

	//Set our autostart state
	m_bAutoStart = !!( m_spawnflags & SF_REBEL_TURRET_AUTOACTIVATE );
	m_bEnabled	 = ( ( m_spawnflags & SF_REBEL_TURRET_STARTINACTIVE ) == false );

	//Do we start active?
	if ( m_bAutoStart && m_bEnabled )
	{
		SetThink( &CNPC_RebelTurret::AutoSearchThink );
		SetEyeState( TURRET_EYE_DORMANT );
	}
	else
	{
		SetThink( &CNPC_RebelTurret::DisabledThink );
	...
	SetUse( &CNPC_RebelTurret::ToggleUse );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::Activate( void )
{
	BaseClass::Activate();
...
bool CNPC_RebelTurret::CreateVPhysics( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::Retire( void )
{
	...
	if ( GetActivity() != ACT_REBEL_TURRET_CLOSE )
	{
		//Set our visible state to dormant
		SetEyeState( TURRET_EYE_DORMANT );

		SetActivity( (Activity) ACT_REBEL_TURRET_OPEN_IDLE );
		
		//If we're done moving to our desired facing, close up
		if ( UpdateFacing() == false )
		{
			SetActivity( (Activity) ACT_REBEL_TURRET_CLOSE );
			EmitSound( "NPC_RebelTurret.Retire" );
	...
	{	
		...
		SetActivity( (Activity) ACT_REBEL_TURRET_CLOSED_IDLE );
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::Deploy( void )
{
	...
	if ( GetActivity() != ACT_REBEL_TURRET_OPEN )
	{
		m_bActive = true;
		SetActivity( (Activity) ACT_REBEL_TURRET_OPEN );
		EmitSound( "NPC_RebelTurret.Deploy" );
	...
	if ( IsActivityFinished() )
	{
		...
		m_flPlaybackRate = 0;
		SetThink( &CNPC_RebelTurret::SearchThink );

		EmitSound( "NPC_RebelTurret.Move" );
	}

	m_flLastSight = gpGlobals->curtime + REBEL_TURRET_MAX_WAIT;	
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::OnPhysGunDrop( CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason )
{
	m_hPhysicsAttacker = pPhysGunUser;
	m_flLastPhysicsInfluenceTime = gpGlobals->curtime;
...
//-----------------------------------------------------------------------------
bool CNPC_RebelTurret::HasPreferredCarryAnglesForPlayer( CBasePlayer *pPlayer )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_RebelTurret::OnAttemptPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_RebelTurret::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter *sourceEnt)
{
...
//-----------------------------------------------------------------------------
float CNPC_RebelTurret::MaxYawSpeed( void )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_RebelTurret::WasJustDroppedByPlayer( void )
{
...
//-----------------------------------------------------------------------------
bool CNPC_RebelTurret::UpdateFacing( void )
{
	bool  bMoved = false;
	UpdateMuzzleMatrix();
...
void CNPC_RebelTurret::DryFire( void )
{
	EmitSound( "NPC_RebelTurret.DryFire");
	EmitSound( "NPC_RebelTurret.Activate" );
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::SuppressThink( void )
{
	...
	if ( !GetEnemy() )
	{
		SetThink( &CNPC_RebelTurret::ActiveThink );
		return;
	}
	...
	if ( gpGlobals->curtime > m_flLastSight )
	{
		...
		if ( m_spawnflags & SF_REBEL_TURRET_FASTRETIRE )
		{
			// Retire quickly in this case. (The case where we saw the player, but he hid again).
			m_flLastSight = gpGlobals->curtime + REBEL_TURRET_SHORT_WAIT;
		}
		else
		{
			m_flLastSight = gpGlobals->curtime + REBEL_TURRET_MAX_WAIT;
	...
	if ( m_flShotTime < gpGlobals->curtime && m_vecEnemyLKP != vec3_invalid )
	{
		...
		{
			if( m_spawnflags & SF_REBEL_TURRET_OUT_OF_AMMO )
			{
				DryFire();
			}
			else
			{
				ResetActivity();
				SetActivity( (Activity) ACT_REBEL_TURRET_FIRE );
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::ActiveThink( void )
{
	//Allow descended classes a chance to do something before the think function
	...
	if ( ( bEnemyVisible == false ) || ( flDistToEnemy > REBEL_TURRET_RANGE ))
	{
		m_flLastSight = gpGlobals->curtime + 2.0f;

		ClearEnemyMemory();
		SetEnemy( NULL );
		SetThink( &CNPC_RebelTurret::SuppressThink );
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::SearchThink( void )
{
	//Allow descended classes a chance to do something before the think function
 	...
 	if ( gpGlobals->curtime > m_flLastSight )
	{
		...
		m_flLastSight = 0;
		SetThink( &CNPC_RebelTurret::Retire );
		return;
	}
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::AutoSearchThink( void )
{
	//Allow descended classes a chance to do something before the think function
	...
	if ( GetEnemy() != NULL )
	{
		SetThink( &CNPC_RebelTurret::Deploy );
		if ( !m_bNoAlarmSounds )
		{
			EmitSound( "NPC_RebelTurret.Alert" );
		}
	}
}
...
// Purpose: Fire!
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy, bool bStrict )
{
	...
	FireBullets( info );
	EmitSound( "NPC_RebelTurret.ShotSounds", m_ShotSounds );
	DoMuzzleFlash();
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_RebelTurret::IsValidEnemy( CBaseEntity *pEnemy )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_RebelTurret::CanBeAnEnemyOf( CBaseEntity *pEnemy )
{
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::TippedThink( void )
{
	...
	else
	{
		...
		if ( GetActivity() != ACT_REBEL_TURRET_CLOSE )
		{
			...
			if ( UpdateFacing() == false )
			{
				...
				EmitSound( "NPC_RebelTurret.Die" );
				SpinDown();

				SetActivity( (Activity) ACT_REBEL_TURRET_CLOSE );
				EmitSound( "NPC_RebelTurret.Retract" );
		...
		{	
			...
			if ( UpdateFacing() == false )
			{
				...
				SetThink( &CNPC_RebelTurret::InactiveThink );
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::InactiveThink( void )
{
	// Update our PVS state
	CheckPVSCondition();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::ReturnToLife( void )
{
	m_flThrashTime = 0;
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::DisabledThink( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::HackFindEnemy( void )
{
	...
	GetSenses()->Look( REBEL_TURRET_RANGE );
	SetEnemy( BestEnemy() );
}
...
//-----------------------------------------------------------------------------
inline bool CNPC_RebelTurret::OnSide( void )
{
	Vector	up;
...
//-----------------------------------------------------------------------------
bool CNPC_RebelTurret::PreThink( turretState_e state )
{
	...
	if ( !IsBeingCarriedByPlayer() )
	{
		...
		{
			if ( HasSpawnFlags( SF_REBEL_TURRET_OUT_OF_AMMO ) == false )
			{
				//Thrash around for a bit
				m_flThrashTime = gpGlobals->curtime + random->RandomFloat( 2.0f, 2.5f );
				SetNextThink( gpGlobals->curtime + 0.05f );

				SetThink( &CNPC_RebelTurret::TippedThink );
				SetEyeState( TURRET_EYE_SEE_TARGET );
				
				SpinUp();
				if ( !m_bNoAlarmSounds )
				{
					EmitSound( "NPC_RebelTurret.Alarm" );
				}
			}
			else
			{
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::SetEyeState( eyeState_t state )
{
	// Must have a valid eye to affect
	if ( !m_hEyeGlow )
	{
		// Create our eye sprite
		m_hEyeGlow = CSprite::SpriteCreate( REBEL_TURRET_GLOW_SPRITE, GetLocalOrigin(), false );
		if ( !m_hEyeGlow )
			return;
	...
	if ( IsCitizenTurret() && HasSpawnFlags( SF_REBEL_TURRET_OUT_OF_AMMO ) == false && m_hLaser == NULL )
	{
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::Ping( void )
{
	...
	//Ping!
	EmitSound( "NPC_RebelTurret.Ping" );

	SetEyeState( TURRET_EYE_SEEKING_TARGET );

	m_flPingTime = gpGlobals->curtime + REBEL_TURRET_PING_TIME;
}

//-----------------------------------------------------------------------------
// Purpose: Toggle the turret's state
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::Toggle( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::Enable( void )
{
	...
	if ( m_spawnflags & SF_REBEL_TURRET_AUTOACTIVATE )
	{
		m_bAutoStart = true;
	}
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::Disable( void )
{
	//This turret is on its side, it can't function
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::InputToggle( inputdata_t &inputdata )
{
	Toggle();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::InputEnable( inputdata_t &inputdata )
{
	Enable();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::InputDisable( inputdata_t &inputdata )
{
	Disable();
}
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::InputDepleteAmmo( inputdata_t &inputdata )
{
	AddSpawnFlags( SF_REBEL_TURRET_OUT_OF_AMMO );
}

//-----------------------------------------------------------------------------
// Purpose: Allows the turret to fire live rounds again
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::InputRestoreAmmo( inputdata_t &inputdata )
{
	RemoveSpawnFlags( SF_REBEL_TURRET_OUT_OF_AMMO );
}

//-----------------------------------------------------------------------------
// Purpose: Allow players and npc's to turn the turret on and off
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::ToggleUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	switch( useType )
	{
	case USE_OFF:
...
//-----------------------------------------------------------------------------
int CNPC_RebelTurret::VPhysicsTakeDamage( const CTakeDamageInfo &info )
{
...
// Input  : &info - 
//-----------------------------------------------------------------------------
int CNPC_RebelTurret::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( IsAlive() && m_bEnabled && m_bAutoStart && GetActivity() == ACT_REBEL_TURRET_CLOSED_IDLE && m_bSelfDestructing == false )
	{
		SetThink( &CNPC_RebelTurret::Deploy );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::SpinUp( void )
{
}

#define	REBEL_TURRET_MIN_SPIN_DOWN	1.0f

//-----------------------------------------------------------------------------
// Purpose: 
// Output : const QAngle
//-----------------------------------------------------------------------------
QAngle CNPC_RebelTurret::PreferredCarryAngles( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::SpinDown( void )
{
}
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_RebelTurret::GetAttackDamageScale( CBaseEntity *pVictim )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
Vector CNPC_RebelTurret::GetAttackSpread( CBaseCombatWeapon *pWeapon, CBaseEntity *pTarget ) 
{
...
//------------------------------------------------------------------------------
CBasePlayer *CNPC_RebelTurret::HasPhysicsAttacker( float dt )
{
...
//-----------------------------------------------------------------------------
int CNPC_RebelTurret::DrawDebugTextOverlays( void ) 
{
...
void CNPC_RebelTurret::UpdateMuzzleMatrix()
{
...
//			reappear.
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::PlayerPenetratingVPhysics( void )
{
	// We don't care!
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::BreakThink( void )
{
	Vector vecUp;
	...
	EmitSound( "NPC_RebelTurret.Destruct" );
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::SelfDestructThink( void )
{
	// Continue to animate
	...
	{
		SetThink( &CNPC_RebelTurret::BreakThink );
		SetNextThink( gpGlobals->curtime + 0.1f );
	...
	if ( gpGlobals->curtime > ( m_flPingTime + flBeepTime ) )
	{
		...
		StopSound( "NPC_RebelTurret.AlarmPing" );

		// Play the beep
		CPASAttenuationFilter filter( this, "NPC_RebelTurret.AlarmPing" );
		EmitSound_t params;
		params.m_pSoundName = "NPC_RebelTurret.AlarmPing";
...
//-----------------------------------------------------------------------------
void CNPC_RebelTurret::InputSelfDestruct( inputdata_t &inputdata )
{
	// Ka-boom!
	...
	SetThink( &CNPC_RebelTurret::SelfDestructThink );
	SetNextThink( gpGlobals->curtime + 0.1f );
...
IMotionEvent::simresult_e CAssTipCountroller::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	...
	else
	{
		...
		if ( (vecVelocity.LengthSqr() > CNPC_RebelTurret::fMaxTipControllerVelocity) || (angImpulse.LengthSqr() > CNPC_RebelTurret::fMaxTipControllerAngularVelocity) )
			return SIM_NOTHING;
	}
...
//
//-----------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC( npc_turret_floor, CNPC_RebelTurret )

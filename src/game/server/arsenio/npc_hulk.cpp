...
#define	COMBINEGUARD_MODEL	"models/arsenio/npc/hulk.mdl"

#define HULK_MSG_SHOT	1
#define HULK_MSG_SHOT_START 2

#define		GUARD_BODY_GUNDRAWN		0
#define		GUARD_BODY_GUNGONE			1


class CNPC_Hulk : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_Hulk, CAI_BaseNPC );
public:
...
#define	HULK_DEFAULT_ARMOR_HEALTH	50
...
#define	HULK_GLOW_TIME			0.5f

BEGIN_DATADESC( CNPC_Hulk )
...
enum HulkSchedules 
{	
	SCHED_HULK_RANGE_ATTACK1 = LAST_SHARED_SCHEDULE,

};

enum HulkTasks 
{	
	TASK_HULK_RANGE_ATTACK1 = LAST_SHARED_TASK,
...
int	ACT_HULK_IDLE_TO_ANGRY;
int ACT_COMBINEGUARD_CLOBBERED;
int ACT_COMBINEGUARD_TOPPLE;
int ACT_COMBINEGUARD_GETUP;
int ACT_COMBINEGUARD_HELPLESS;

#define	HULK_AE_SHOVE	11
#define	HULK_AE_FIRE 12
#define	HULK_AE_FIRE_START 13
#define	HULK_AE_GLOW 14
#define HULK_AE_LEFTFOOT 20
#define HULK_AE_RIGHTFOOT	21
#define HULK_AE_SHAKEIMPACT 22

CNPC_Hulk::CNPC_Hulk( void )
{
}

LINK_ENTITY_TO_CLASS( npc_hulk, CNPC_Hulk );

void CNPC_Hulk::Precache( void )
{
	...
	PrecacheScriptSound( "NPC_Hulk.FootstepLeft" );
...
void CNPC_Hulk::Spawn( void )
{
	...
	m_iHealth = m_iMaxHealth = sk_hulk_health.GetFloat();
...
void CNPC_Hulk::PrescheduleThink( void )
{
...
void CNPC_Hulk::Event_Killed( const CTakeDamageInfo &info )
{
...
int CNPC_Hulk::SelectSchedule ( void )
{
	{
...
void CNPC_Hulk::HandleAnimEvent( animevent_t *pEvent )
{
	switch ( pEvent -> event )
	{
	case HULK_AE_SHAKEIMPACT:
		Shove();
		UTIL_ScreenShake( GetAbsOrigin(), 25.0, 150.0, 1.0, 750, SHAKE_START );
		break;

	case HULK_AE_LEFTFOOT:
		{
			EmitSound( "NPC_Hulk.FootstepLeft" );
		}
		break;

	case HULK_AE_RIGHTFOOT:
		{
			EmitSound( "NPC_Hulk.FootstepRight" );
		}
		break;
	...
	case HULK_AE_FIRE:
		{
			...
			EmitSound( "NPC_Hulk.Fire" );

			EntityMessageBegin( this, true );
				WRITE_BYTE( HULK_MSG_SHOT );
			MessageEnd();
		}
		break;

	case HULK_AE_FIRE_START:
		{
			EmitSound( "NPC_Hulk.Charging" );

			EntityMessageBegin( this, true );
				WRITE_BYTE( HULK_MSG_SHOT_START );
			MessageEnd();
		}
		break;

	case HULK_AE_GLOW:
		m_flGlowTime = gpGlobals->curtime + HULK_GLOW_TIME;
		break;
...
void CNPC_Hulk::Shove( void ) // Doesn't work for some reason // It sure works now! -Dan
{
	if ( GetEnemy() == NULL )
		return;
...
void CNPC_Hulk::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
void CNPC_Hulk::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_HULK_RANGE_ATTACK1:
...
Activity CNPC_Hulk::NPC_TranslateActivity( Activity baseAct )
{
	if( baseAct == ACT_RUN )
	{
...
int CNPC_Hulk::TranslateSchedule( int type ) 
{
	switch( type )
	{
	case SCHED_RANGE_ATTACK1:
		return SCHED_HULK_RANGE_ATTACK1;
		break;
	}
...
int CNPC_Hulk::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
int CNPC_Hulk::RangeAttack1Conditions( float flDot, float flDist )
{
...
void CNPC_Hulk::DoMuzzleFlash( void )
{
...
void CNPC_Hulk::FireRangeWeapon( void )
{
...
bool CNPC_Hulk::AimGunAt( CBaseEntity *pEntity, float flInterval )
{
	if ( !pEntity )
		return true;
...
float CNPC_Hulk::MaxYawSpeed( void ) 
{ 	
...
void CNPC_Hulk::BuildScheduleTestBits( void )
{
	SetCustomInterruptCondition( COND_COMBINEGUARD_CLOBBERED );
}

AI_BEGIN_CUSTOM_NPC( npc_hulk, CNPC_Hulk )

	DECLARE_TASK( TASK_HULK_RANGE_ATTACK1 )
	DECLARE_TASK( TASK_COMBINEGUARD_SET_BALANCE )
	
	DECLARE_CONDITION( COND_COMBINEGUARD_CLOBBERED )

	DECLARE_ACTIVITY( ACT_HULK_IDLE_TO_ANGRY )
	...
	DEFINE_SCHEDULE
	(
		SCHED_HULK_RANGE_ATTACK1,

		"	Tasks"
		"		TASK_STOP_MOVING			0"
		"		TASK_FACE_ENEMY				0"
		"		TASK_ANNOUNCE_ATTACK		1"
		"		TASK_HULK_RANGE_ATTACK1	0"
		"	"
		"	Interrupts"
		"		COND_NEW_ENEMY"
		"		COND_ENEMY_DEAD"

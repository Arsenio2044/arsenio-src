...
//
// Purpose: The Hellscraper is one of the leaders of SHADOW, in Arsenio 3044 it serves as a Boss fight.
//=============================================================================//
#include	"cbase.h"
...
#ifdef AR
#include "arsenio_baseshadownpc.h"
#endif

ConVar sk_hellscraper_health_factor( "sk_hellscraper_health_factor", "1" );
ConVar sk_hellscraper_dmg_slash( "sk_hellscraper_dmg_slash", "50" );
ConVar sk_hellscraper_dmg_blast( "sk_hellscraper_dmg_blast", "100" );
ConVar sk_hellscraper_radius_blast( "sk_hellscraper_radius_blast", "250" );
...
LINK_ENTITY_TO_CLASS( info_hellscraper, CInfoBM );
...
public:
	...
	Class_T	Classify( void ) { return CLASS_SHADOW; };
	...
	const char *GetNodeSequence( void )
	{
		...
		if ( pTarget && FClassnameIs( pTarget, "info_hellscraper" ) )
		{
...
BEGIN_DATADESC( CNPC_HellScraper )
...
LINK_ENTITY_TO_CLASS ( monster_hellscraper, CNPC_HellScraper );

//=========================================================
// Spawn
//=========================================================
void CNPC_HellScraper::Spawn()
{
	Precache( );
	...
	m_iHealth = 150 * sk_hellscraper_health_factor.GetFloat();
...
//=========================================================
void CNPC_HellScraper::Precache()
{
	...
	PrecacheScriptSound( "HellScraper.Pain" );
	PrecacheScriptSound( "HellScraper.Attack" );
	PrecacheScriptSound( "HellScraper.AttackHit" );
	PrecacheScriptSound( "HellScraper.Alert" );
	PrecacheScriptSound( "HellScraper.Birth" );
	PrecacheScriptSound( "HellScraper.Sack" );
	PrecacheScriptSound( "HellScraper.Die" );
	PrecacheScriptSound( "HellScraper.FootstepLeft" );
	PrecacheScriptSound( "HellScraper.FootstepRight" );
	PrecacheScriptSound( "HellScraper.LayHeadcrab" );
	PrecacheScriptSound( "HellScraper.ChildDie" );
	PrecacheScriptSound( "HellScraper.LaunchMortar" );
}
...
//=========================================================
float CNPC_HellScraper::MaxYawSpeed ( void )
{
	float ys = 90.0f;
...
void CNPC_HellScraper::Activate( void )
{
	if ( GetTarget() == NULL )
...
void CNPC_HellScraper::NodeStart( string_t iszNextNode )
{
...
void CNPC_HellScraper::NodeReach( void )
{
	CInfoBM *pTarget = (CInfoBM*)GetTarget();
	...
	if ( pTarget->m_iHealth >= 1 )
		 m_iMaxHealth = m_iHealth = pTarget->m_iHealth * sk_hellscraper_health_factor.GetFloat();
...
void CNPC_HellScraper::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
...
int CNPC_HellScraper::OnTakeDamage( const CTakeDamageInfo &info )
{
	CTakeDamageInfo newInfo = info;

	// Don't take any acid damage -- HellScraper's mortar is acid
...
bool CNPC_HellScraper::ShouldGoToNode( void )
{
...
int CNPC_HellScraper::SelectSchedule( void )
{
	if ( ShouldGoToNode() )
	{
...
void CNPC_HellScraper::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	...
	case TASK_MELEE_ATTACK1:
		...
		// Play an attack sound here
			...
			EmitSound( filter, entindex(), "HellScraper.Attack" );
...
// RunTask
//=========================================================
void CNPC_HellScraper::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_CHECK_NODE_PROXIMITY:
...
//=========================================================
void CNPC_HellScraper::HandleAnimEvent( animevent_t *pEvent )
{
	CPASAttenuationFilter filter( this );
...
//						pHurt->pev->velocity = pHurt->pev->velocity + (vecFwd * 150) + Vector(0,0,250) - (vecRight * 200);
				...
				EmitSound( filter, entindex(), "HellScraper.AttackHit" );
			}
		}
		break;
		
		case BIG_AE_SCREAM:
			EmitSound( filter, entindex(), "HellScraper.Alert" );
			break;
		
		case BIG_AE_PAIN_SOUND:
			EmitSound( filter, entindex(), "HellScraper.Pain" );
			break;
		...
		case BIG_AE_BIRTH_SOUND:
			EmitSound( filter, entindex(), "HellScraper.Birth" );
			break;

		case BIG_AE_SACK:
			if ( RandomInt(0,100) < 30 )
			{
				EmitSound( filter, entindex(), "HellScraper.Sack" );
			}
			break;

		case BIG_AE_DEATHSOUND:
			EmitSound( filter, entindex(), "HellScraper.Die" );
			break;

		case BIG_AE_STEP1:		// Footstep left
		case BIG_AE_STEP3:		// Footstep back left
			EmitSound( filter, entindex(), "HellScraper.FootstepLeft" );
			break;

		case BIG_AE_STEP4:		// Footstep back right
		case BIG_AE_STEP2:		// Footstep right
			EmitSound( filter, entindex(), "HellScraper.FootstepRight" );
			break;
...
void CNPC_HellScraper::LayHeadcrab( void )
{
	...
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "HellScraper.LayHeadcrab" );

	m_crabCount++;
}

void CNPC_HellScraper::DeathNotice( CBaseEntity *pevChild )
{
	...
	if ( IsAlive() )
	{
		...
		CPASAttenuationFilter filter( this );
		EmitSound( filter, entindex(), "HellScraper.ChildDie" );
	}
}


void CNPC_HellScraper::LaunchMortar( void )
{
	...
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "HellScraper.LaunchMortar" );
...
int CNPC_HellScraper::MeleeAttack1Conditions( float flDot, float flDist )
{
	if (flDot >= 0.7)
	{
...
// Lay a crab
int CNPC_HellScraper::MeleeAttack2Conditions( float flDot, float flDist )
{
	return CanLayCrab();
}
...
// Mortar launch
int CNPC_HellScraper::RangeAttack1Conditions( float flDot, float flDist )
{
...
void CBMortar::Precache()
{
	...
	PrecacheScriptSound( "NPC_HellScraper.SpitTouch1" );
	PrecacheScriptSound( "NPC_HellScraper.SpitHit1" );
	PrecacheScriptSound( "NPC_HellScraper.SpitHit2" );
}
...
void CBMortar::Touch( CBaseEntity *pOther )
{
	...
	EmitSound( "NPC_HellScraper.SpitTouch1" );

	switch ( random->RandomInt( 0, 1 ) )
	{
	case 0:
		EmitSound( "NPC_HellScraper.SpitHit1" );
		break;
	case 1:
		EmitSound( "NPC_HellScraper.SpitHit2" );
		break;
	}
	...
	RadiusDamage( CTakeDamageInfo( this, pOwner, sk_hellscraper_dmg_blast.GetFloat(), DMG_ACID ), GetAbsOrigin(), sk_hellscraper_radius_blast.GetFloat(), CLASS_NONE, NULL );
...
AI_BEGIN_CUSTOM_NPC( monster_hellscraper, CNPC_HellScraper )

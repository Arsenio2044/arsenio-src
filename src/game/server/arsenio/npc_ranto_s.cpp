...
ConVar	arsenio_rantos_heal_player_min_pct	( "arsenio_rantos_heal_player_min_pct",		"0.60");
ConVar	arsenio_rantos_heal_player_min_forced( "arsenio_rantos_heal_player_min_forced",		"10.0");
ConVar	arsenio_rantos_heal_ally			( "arsenio_rantos_heal_ally",				"30");
ConVar	arsenio_rantos_heal_ally_delay		( "arsenio_rantos_heal_ally_delay",			"20");
ConVar	arsenio_rantos_heal_ally_min_pct	( "arsenio_rantos_heal_ally_min_pct",		"0.90");
ConVar	arsenio_rantos_player_stare_time	( "arsenio_rantos_player_stare_time",		"1.0" );
ConVar  arsenio_rantos_player_stare_dist	( "arsenio_rantos_player_stare_dist",		"72" );
ConVar	arsenio_rantos_stare_heal_time		( "arsenio_rantos_stare_heal_time",			"5" );

ConVar	g_ai_rantos_show_enemy( "g_ai_rantos_show_enemy", "0" );

ConVar	npc_ranto_s_insignia( "npc_ranto_s_insignia", "0" );
ConVar	npc_ranto_s_squad_marker( "npc_ranto_s_squad_marker", "0" );
ConVar	npc_ranto_s_explosive_resist( "npc_ranto_s_explosive_resist", "0" );
ConVar	npc_ranto_s_auto_player_squad( "npc_ranto_s_auto_player_squad", "1" );
ConVar	npc_ranto_s_auto_player_squad_allow_use( "npc_ranto_s_auto_player_squad_allow_use", "0" );

ConVar	npc_ranto_s_dont_precache_all( "npc_ranto_s_dont_precache_all", "0" );


ConVar  npc_ranto_s_medic_emit_sound("npc_ranto_s_medic_emit_sound", "1" );
#ifdef HL2_EPISODIC
// todo: bake these into pound constants (for now they're not just for tuning purposes)
ConVar  npc_ranto_s_heal_chuck_medkit("npc_ranto_s_heal_chuck_medkit" , "1" , FCVAR_ARCHIVE, "Set to 1 to use new experimental healthkit-throwing medic.");
ConVar npc_ranto_s_medic_throw_style( "npc_ranto_s_medic_throw_style", "1", FCVAR_ARCHIVE, "Set to 0 for a lobbier trajectory" );
ConVar npc_ranto_s_medic_throw_speed( "npc_ranto_s_medic_throw_speed", "650" );
ConVar	arsenio_rantos_heal_toss_player_delay("arsenio_rantos_heal_toss_player_delay", "26", FCVAR_NONE, "how long between throwing healthkits" );


#define MEDIC_THROW_SPEED npc_ranto_s_medic_throw_speed.GetFloat()
#define USE_EXPERIMENTAL_MEDIC_CODE() (npc_ranto_s_heal_chuck_medkit.GetBool() && NameMatches("griggs"))
#endif
...
#define ShouldAutosquad() (npc_ranto_s_auto_player_squad.GetBool())
...
// Animation events
int AE_RANTOS_GET_PACKAGE;
int AE_RANTOS_HEAL;

//-------------------------------------
//-------------------------------------

ConVar	rantos_follow_move_commands( "rantos_follow_move_commands", "1" );
ConVar	ai_rantos_debug_commander( "ai_rantos_debug_commander", "1" );
#define DebuggingCommanderMode() (ai_rantos_debug_commander.GetBool() && (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))

//-----------------------------------------------------------------------------
// Rantos expressions for the citizen expression types
//-----------------------------------------------------------------------------
#define STATES_WITH_EXPRESSIONS		3		// Idle, Alert, Combat
#define EXPRESSIONS_PER_STATE		1

char *szExpressionTypeRantoss[CIT_EXP_LAST_TYPE] =
{
	"Unassigned",
	"Scared",
	"Normal",
...
struct rantos_expression_list_t
{
	char *szExpressions[EXPRESSIONS_PER_STATE];
};
// Scared
rantos_expression_list_t ScaredExpressions[STATES_WITH_EXPRESSIONS] =
{
...
// Normal
rantos_expression_list_t NormalExpressions[STATES_WITH_EXPRESSIONS] =
{
...
// Angry
rantos_expression_list_t AngryExpressions[STATES_WITH_EXPRESSIONS] =
{
...
int ACT_RANTOS_HANDSUP;
int	ACT_RANTOS_BLINDED;		// Blinded by scanner photo
int ACT_RANTOS_SHOWARMBAND;
int ACT_RANTOS_HEAL;
int	ACT_RANTOS_STARTLED;		// Startled by sneaky scanner

//---------------------------------------------------------

LINK_ENTITY_TO_CLASS( npc_ranto_s, CNPC_RANTOS );

//---------------------------------------------------------

BEGIN_DATADESC( CNPC_RANTOS )
...
//						m_FuncTankBehavior
	...
	DEFINE_KEYFIELD(	m_ExpressionTypeRantos,			FIELD_INTEGER,	"expressiontype" ),
...
CSimpleSimTimer CNPC_RANTOS::gm_PlayerSquadEvaluateTimer;

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------

bool CNPC_RANTOS::CreateBehaviors()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::Precache()
{
	SelectModel();
	SelectExpressionTypeRantos();

	if ( !npc_ranto_s_dont_precache_all.GetBool() )
	...
	PrecacheScriptSound( "NPC_Rantos.FootstepLeft" );
	PrecacheScriptSound( "NPC_Rantos.FootstepRight" );
	PrecacheScriptSound( "NPC_Rantos.Die" );

	PrecacheInstancedScene( "scenes/Expressions/RantosIdle.vcd" );
	PrecacheInstancedScene( "scenes/Expressions/RantosAlert_loop.vcd" );
	PrecacheInstancedScene( "scenes/Expressions/RantosCombat_loop.vcd" );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::PrecacheAllOfType( RantosType_t type )
{
	if ( m_Type == CT_UNIQUE )
		return;
	...
	for ( i = 0; i < nHeads; ++i )
	{
		...
		{
			PrecacheModel( CFmtStr( "models/ranto/%s/%s", (const char *)(CFmtStr(g_ppszModelLocs[m_Type], "")), g_ppszRandomHeads[i] ) );
		}
	}
	...
	if ( m_Type == CT_REBEL )
	{
		for ( i = 0; i < nHeads; ++i )
		{
			...
			{
				PrecacheModel( CFmtStr( "models/ranto/%s/%s", (const char *)(CFmtStr(g_ppszModelLocs[m_Type], "m")), g_ppszRandomHeads[i] ) );
			}
		}
	}
}
...
void CNPC_RANTOS::Spawn()
{
	...
	m_iHealth = arsenio_rantos_health.GetFloat();
	...
	SetUse( &CNPC_RANTOS::CommanderUse );
...
void CNPC_RANTOS::SelectModel()
{
	...
	{
		struct RantosTypeMapping
		{
			const char *pszMapTag;
			RantosType_t type;
		};

		static RantosTypeMapping RantosTypeMappings[] = 
		{
		...
		for ( int i = 0; i < ARRAYSIZE(RantosTypeMappings); i++ )
		{
			if ( Q_stristr( szMapName, RantosTypeMappings[i].pszMapTag ) )
			{
				m_Type = RantosTypeMappings[i].type;
				break;
			}
		}
	...
	if( HasSpawnFlags( SF_CITIZEN_RANDOM_HEAD | SF_CITIZEN_RANDOM_HEAD_MALE | SF_CITIZEN_RANDOM_HEAD_FEMALE ) || GetModelName() == NULL_STRING )
	{
		...
		{
			SetModelName( AllocPooledString("models/ranto/male_cheaple.mdl" ) );
			return;
		}
		else
		{
			...
			{
				CNPC_RANTOS *pRantos = dynamic_cast<CNPC_RANTOS *>(g_AI_Manager.AccessAIs()[i]);
				if ( pRantos && pRantos != this && pRantos->m_iHead >= 0 && pRantos->m_iHead < ARRAYSIZE(g_ppszRandomHeads) )
				{
					headCounts[pRantos->m_iHead]++;
				}
			}
	...
	if ( m_Type != CT_UNIQUE )
	{
		SetModelName( AllocPooledString( CFmtStr( "models/ranto/%s/%s", (const char *)(CFmtStr(g_ppszModelLocs[ m_Type ], ( IsMedic() ) ? "m" : "" )), pszModelName ) ) );
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RANTOS::SelectExpressionTypeRantos()
{
	// If we've got a mapmaker assigned type, leave it alone
	if ( m_ExpressionTypeRantos != CIT_EXP_UNASSIGNED )
		return;

	switch ( m_Type )
	{
	case CT_DOWNTRODDEN:
		m_ExpressionTypeRantos = (RantosExpressionTypeRantoss_t)RandomInt( CIT_EXP_SCARED, CIT_EXP_NORMAL );
		break;
	case CT_REFUGEE:
		m_ExpressionTypeRantos = (RantosExpressionTypeRantoss_t)RandomInt( CIT_EXP_SCARED, CIT_EXP_NORMAL );
		break;
	case CT_REBEL:
		m_ExpressionTypeRantos = (RantosExpressionTypeRantoss_t)RandomInt( CIT_EXP_SCARED, CIT_EXP_ANGRY );
		break;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::FixupMattWeapon()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
string_t CNPC_RANTOS::GetModelName() const
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::ShouldAlwaysThink() 
{ 
...
bool CNPC_RANTOS::ShouldBehaviorSelectSchedule( CAI_BehaviorBase *pBehavior )
{
...
void CNPC_RANTOS::OnChangeRunningBehavior( CAI_BehaviorBase *pOldBehavior,  CAI_BehaviorBase *pNewBehavior )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::GatherConditions()
{
	...
	if ( !SpokeConcept( TLK_JOINPLAYER ) && IsRunningScriptedSceneWithSpeech( this, true ) )
	{
		...
		for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			...
			{
				(assert_cast<CNPC_RANTOS *>(pNpc))->SetSpokeConcept( TLK_JOINPLAYER, NULL );
			}
		}
	}
	...
	if( CanHeal() )
	{
		...
		float flStareDist = arsenio_rantos_player_stare_dist.GetFloat();
		...
		if( pPlayer->IsAlive() && flPlayerDamage > 0 && (flDistSqr <= flStareDist * flStareDist) && pPlayer->FInViewCone( this ) && pPlayer->FVisible( this ) )
		{
			...
			if( gpGlobals->curtime - m_flTimePlayerStare >= arsenio_rantos_player_stare_time.GetFloat() && gpGlobals->curtime > m_flTimeNextHealStare && !IsCurSchedule( SCHED_CITIZEN_HEAL ) )
			{
				...
				{
					m_flTimeNextHealStare = gpGlobals->curtime + arsenio_rantos_stare_heal_time.GetFloat() * .5f;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::PredictPlayerPush()
{
	if ( !AI_IsSinglePlayer() )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::PrescheduleThink()
{
	...
	if ( !npc_ranto_s_insignia.GetBool() && npc_ranto_s_squad_marker.GetBool() && IsInPlayerSquad() )
	{
	...
	if( GetEnemy() && g_ai_rantos_show_enemy.GetBool() )
	{
...
//-----------------------------------------------------------------------------
void CNPC_RANTOS::BuildScheduleTestBits()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::FInViewCone( CBaseEntity *pEntity )
{
#if 0
	if ( IsMortar( pEntity ) )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RANTOS::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RANTOS::SelectSchedule()
{
	...
	{
		DevMsg( "Rantos in select schedule but RPG is guiding?\n");
		pRPG->StopGuiding();
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RANTOS::SelectSchedulePriorityAction()
{
...
//-----------------------------------------------------------------------------
int CNPC_RANTOS::SelectScheduleHeal()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RANTOS::SelectScheduleRetrieveItem()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RANTOS::SelectScheduleNonCombat()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RANTOS::SelectScheduleManhackCombat()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RANTOS::SelectScheduleCombat()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::ShouldDeferToFollowBehavior()
{
#if 0
	if ( HaveCommandGoal() )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RANTOS::TranslateSchedule( int scheduleType ) 
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::ShouldAcceptGoal( CAI_BehaviorBase *pBehavior, CAI_GoalEntity *pGoal )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::OnClearGoal( CAI_BehaviorBase *pBehavior, CAI_GoalEntity *pGoal )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	...
	case TASK_CIT_HEAL_TOSS:
		...
		SetIdealActivity( (Activity)ACT_RANTOS_HEAL );
		break;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
...
// Input  : code - 
//-----------------------------------------------------------------------------
void CNPC_RANTOS::TaskFail( AI_TaskFailureCode_t code )
{
	// If our heal task has failed, push out the heal time
	if ( IsCurSchedule( SCHED_CITIZEN_HEAL ) )
	{
		m_flPlayerHealTime 	= gpGlobals->curtime + arsenio_rantos_heal_ally_delay.GetFloat();
	}
...
//-----------------------------------------------------------------------------
Activity CNPC_RANTOS::NPC_TranslateActivity( Activity activity )
{
	...
	// !!!HACK - Rantoss don't have the required animations for shotguns, 
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_RANTOS::HandleAnimEvent( animevent_t *pEvent )
{
	if ( pEvent->event == AE_RANTOS_GET_PACKAGE )
	{
	...
	else if ( pEvent->event == AE_RANTOS_HEAL )
	{
		...
		if ( USE_EXPERIMENTAL_MEDIC_CODE() && IsMedic() )
		{
			...
			if ( pTarget )
			{
				m_flPlayerHealTime 	= gpGlobals->curtime + arsenio_rantos_heal_toss_player_delay.GetFloat();;
	...
	case NPC_EVENT_LEFTFOOT:
		{
			EmitSound( "NPC_Rantos.FootstepLeft", pEvent->eventtime );
		}
		break;

	case NPC_EVENT_RIGHTFOOT:
		{
			EmitSound( "NPC_Rantos.FootstepRight", pEvent->eventtime );
		}
		break;
...
#ifdef ARSENIO
void CNPC_RANTOS::Event_Killed(const CTakeDamageInfo& info)
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_RANTOS::PickupItem( CBaseEntity *pItem )
{
	Assert( pItem != NULL );
	...
	{
		DevMsg("Rantos doesn't know how to pick up %s!\n", pItem->GetClassname() );
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::IgnorePlayerPushing( void )
{
...
//-----------------------------------------------------------------------------
const char *CNPC_RANTOS::SelectRandomExpressionForState( NPC_STATE state )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::SimpleUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::OnBeginMoveAndShoot()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::OnEndMoveAndShoot()
{
	VacateStrategySlot();
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::LocateEnemySound()
{
#if 0
	if ( !GetEnemy() )
		return;
	...
	if( flZDiff < -128 )
	{
		EmitSound( "NPC_Rantos.UpThere" );
	}
	else if( flZDiff > 128 )
	{
		EmitSound( "NPC_Rantos.DownThere" );
	}
	else
	{
		EmitSound( "NPC_Rantos.OverHere" );
	}
#endif
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::IsManhackMeleeCombatant()
{
	CBaseCombatWeapon *pWeapon = GetActiveWeapon();
...
//-----------------------------------------------------------------------------
Vector CNPC_RANTOS::GetActualShootPosition( const Vector &shootOrigin )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::OnChangeActiveWeapon( CBaseCombatWeapon *pOldWeapon, CBaseCombatWeapon *pNewWeapon )
{
	if ( pNewWeapon )
	{
...
bool CNPC_RANTOS::ShouldLookForBetterWeapon()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RANTOS::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
#define CITIZEN_SCORCH_FLOOR	75
	...
	if (IsInSquad() && (newInfo.GetDamageType() & DMG_BLAST) && newInfo.GetInflictor())
	{
		if( npc_ranto_s_explosive_resist.GetBool() )
		{
			...
			// single explosion.
			if (m_pSquad->IsSquadInflictor( newInfo.GetInflictor() ))
			{
			...
			else
			{
				...
				{
					m_pSquad->SetSquadInflictor( newInfo.GetInflictor() );
				}
			}
		}
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::IsCommandable() 
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::IsPlayerAlly( CBasePlayer *pPlayer )											
{ 
	if ( Classify() == CLASS_CITIZEN_PASSIVE && GlobalEntity_GetState("gordon_precriminal") == GLOBAL_ON )
	{
		// Robin: Rantoss use friendly speech semaphore in trainstation
		return true;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::CanJoinPlayerSquad()
{
	if ( !AI_IsSinglePlayer() )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::WasInPlayerSquad()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::HaveCommandGoal() const			
{	
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::IsCommandMoving()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::ShouldAutoSummon()
{
...
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::IsValidCommandTarget( CBaseEntity *pTarget )
{
	return false;
}

//-----------------------------------------------------------------------------
bool CNPC_RANTOS::SpeakCommandResponse( AIConcept_t concept, const char *modifiers )
{
...
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::TargetOrder( CBaseEntity *pTarget, CAI_BaseNPC **Allies, int numAllies )
{
	if ( pTarget->IsPlayer() )
	{
...
//-----------------------------------------------------------------------------
void CNPC_RANTOS::MoveOrder( const Vector &vecDest, CAI_BaseNPC **Allies, int numAllies )
{
	if ( !AI_IsSinglePlayer() )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::CommanderUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pActivator == UTIL_GetLocalPlayer() )
	{
		...
		if ( npc_ranto_s_auto_player_squad_allow_use.GetBool() )
		{
			if ( !ShouldAutosquad() )
				TogglePlayerSquadState();
			else if ( !IsInPlayerSquad() && npc_ranto_s_auto_player_squad_allow_use.GetBool() )
				AddToPlayerSquad();
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::ShouldSpeakRadio( CBaseEntity *pListener )
{
	if ( !pListener )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::OnMoveToCommandGoalFailed()
{
	// Clear the goal.
	SetCommandGoal( vec3_invalid );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::AddToPlayerSquad()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::RemoveFromPlayerSquad()
{
	Assert( IsInPlayerSquad() );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::TogglePlayerSquadState()
{
	if ( !AI_IsSinglePlayer() )
		return;
...
void CNPC_RANTOS::UpdatePlayerSquad()
{
	if ( !AI_IsSinglePlayer() )
		return;
	...
	if ( pPlayerSquad )
	{
		CUtlVectorFixed<CNPC_RANTOS *, MAX_PLAYER_SQUAD> squadMembersToRemove;
		AISquadIter_t iter;
		...
		for ( CAI_BaseNPC *pPlayerSquadMember = pPlayerSquad->GetFirstMember(&iter); pPlayerSquadMember; pPlayerSquadMember = pPlayerSquad->GetNextMember(&iter) )
		{
			...
			CNPC_RANTOS *pRantos = assert_cast<CNPC_RANTOS *>(pPlayerSquadMember);

			if ( !pRantos->m_bNeverLeavePlayerSquad &&
				 pRantos->m_FollowBehavior.GetFollowTarget() &&
				 !pRantos->m_FollowBehavior.FollowTargetVisible() && 
				 pRantos->m_FollowBehavior.GetNumFailedFollowAttempts() > 0 && 
				 gpGlobals->curtime - pRantos->m_FollowBehavior.GetTimeFailFollowStarted() > 20 &&
				 ( fabsf(( pRantos->m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().z - pRantos->GetAbsOrigin().z )) > 196 ||
				   ( pRantos->m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().AsVector2D() - pRantos->GetAbsOrigin().AsVector2D() ).LengthSqr() > Square(50*12) ) )
			{
				if ( DebuggingCommanderMode() )
				{
					DevMsg( "Player follower is lost (%d, %f, %d)\n", 
						 pRantos->m_FollowBehavior.GetNumFailedFollowAttempts(), 
						 gpGlobals->curtime - pRantos->m_FollowBehavior.GetTimeFailFollowStarted(), 
						 (int)((pRantos->m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().AsVector2D() - pRantos->GetAbsOrigin().AsVector2D() ).Length()) );
				}

				squadMembersToRemove.AddToTail( pRantos );
			}
		}
	...
	if ( pPlayer && ShouldAutosquad() && !(pPlayer->GetFlags() & FL_NOTARGET ) && pPlayer->IsAlive() )
	{
		...
		for ( i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			...
			CNPC_RANTOS *pRantos = assert_cast<CNPC_RANTOS *>(ppAIs[i]);
			int iNew;

			if ( pRantos->IsInPlayerSquad() )
			{
				iNew = candidates.AddToTail();
				candidates[iNew].pRantos = pRantos;
				candidates[iNew].bIsInSquad = true;
				candidates[iNew].distSq = 0;
				candidates[iNew].iSquadIndex = pRantos->GetSquad()->GetSquadIndex( pRantos );
			}
			else
			{
				float distSq = (vPlayerPos.AsVector2D() - pRantos->GetAbsOrigin().AsVector2D()).LengthSqr(); 
				if ( distSq > JOIN_PLAYER_XY_TOLERANCE_SQ && 
					( pRantos->m_flTimeJoinedPlayerSquad == 0 || gpGlobals->curtime - pRantos->m_flTimeJoinedPlayerSquad > 60.0 ) && 
					( pRantos->m_flTimeLastCloseToPlayer == 0 || gpGlobals->curtime - pRantos->m_flTimeLastCloseToPlayer > 15.0 ) )
					continue;

				if ( !pRantos->CanJoinPlayerSquad() )
					continue;

				bool bShouldAdd = false;

				if ( pRantos->HasCondition( COND_SEE_PLAYER ) )
					bShouldAdd = true;
				else
				{
					bool bPlayerVisible = pRantos->FVisible( pPlayer );
					if ( bPlayerVisible )
					{
						if ( pRantos->HasCondition( COND_HEAR_PLAYER ) )
							bShouldAdd = true;
						else if ( distSq < UNCONDITIONAL_JOIN_PLAYER_XY_TOLERANCE_SQ && fabsf(vPlayerPos.z - pRantos->GetAbsOrigin().z) < UNCONDITIONAL_JOIN_PLAYER_Z_TOLERANCE )
							bShouldAdd = true;
					}
				}
				...
				if ( bShouldAdd )
				{
					...
					AI_Waypoint_t *pPathToPlayer = pRantos->GetPathfinder()->BuildRoute( pRantos->GetAbsOrigin(), vPlayerPos, pPlayer, 5*12, NAV_NONE, true );
		...
		if ( bFoundNewGuy )
		{
			...
			for ( i = 0; i < initialCount; i++ )
				candidates[i].pRantos->AddSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE ); // Prevents double-add
			for ( i = 0; i < initialCount; i++ )
			{
				if ( candidates[i].iSquadIndex == -1 )
				{
					for ( int j = 0; j < g_AI_Manager.NumAIs(); j++ )
					{
						...
						CNPC_RANTOS *pRantos = assert_cast<CNPC_RANTOS *>(ppAIs[j]);

						float distSq = (vPlayerPos - pRantos->GetAbsOrigin()).Length2DSqr(); 
						if ( distSq > JOIN_PLAYER_XY_TOLERANCE_SQ )
							continue;

						distSq = (candidates[i].pRantos->GetAbsOrigin() - pRantos->GetAbsOrigin()).Length2DSqr(); 
						...
						candidates[iNew].distSq = distSq;
						candidates[iNew].iSquadIndex = -1;
						pRantos->AddSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE ); // Prevents double-add
					}
				}
			}
			for ( i = 0; i < candidates.Count(); i++ )
				candidates[i].pRantos->RemoveSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE );
			...
			if ( candidates.Count() > MAX_PLAYER_SQUAD )
			{
				...
				{
					if ( candidates[i].pRantos->IsInPlayerSquad() )
					{
						candidates[i].pRantos->RemoveFromPlayerSquad();
					}
				}
			}
			...
			if ( candidates.Count() )
			{
				...
				for ( i = 0; i < candidates.Count() && i < MAX_PLAYER_SQUAD; i++ )
				{
					if ( !candidates[i].pRantos->IsInPlayerSquad() )
					{
						candidates[i].pRantos->AddToPlayerSquad();
						nJoined++;

						if ( candidates[i].distSq < closestDistSq )
						{
							pClosest = candidates[i].pRantos;
				...
				if ( pClosest )
				{
					...
					for ( i = 0; i < candidates.Count() && i < MAX_PLAYER_SQUAD; i++ )
					{
						candidates[i].pRantos->SetSpokeConcept( TLK_JOINPLAYER, NULL ); 
					}
				}
			}
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RANTOS::PlayerSquadCandidateSortFunc( const SquadCandidate_t *pLeft, const SquadCandidate_t *pRight )
{
	// "Bigger" means less approprate 
	CNPC_RANTOS *pLeftRantos = pLeft->pRantos;
	CNPC_RANTOS *pRightRantos = pRight->pRantos;

	// Medics are better than anyone
	if ( pLeftRantos->IsMedic() && !pRightRantos->IsMedic() )
		return -1;

	if ( !pLeftRantos->IsMedic() && pRightRantos->IsMedic() )
		return 1;

	CBaseCombatWeapon *pLeftWeapon = pLeftRantos->GetActiveWeapon();
	CBaseCombatWeapon *pRightWeapon = pRightRantos->GetActiveWeapon();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::FixupPlayerSquad()
{
	if ( !AI_IsSinglePlayer() )
		return;
	...
	if ( m_pSquad->NumMembers() > MAX_PLAYER_SQUAD )
	{
		...
		CNPC_RANTOS *pFirstMemberRantos = dynamic_cast< CNPC_RANTOS * >( pFirstMember );
		if ( pFirstMemberRantos )
		{
			pFirstMemberRantos->ClearFollowTarget();
		}
		else
		{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::ClearFollowTarget()
{
	m_FollowBehavior.SetFollowTarget( NULL );
	m_FollowBehavior.SetParameters( AIF_SIMPLE );
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::UpdateFollowCommandPoint()
{
	if ( !AI_IsSinglePlayer() )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::IsFollowingCommandPoint()
{
	CBaseEntity *pFollowTarget = m_FollowBehavior.GetFollowTarget();
	if ( pFollowTarget )
...
CAI_BaseNPC *CNPC_RANTOS::GetSquadCommandRepresentative()
{
	if ( !AI_IsSinglePlayer() )
		return NULL;
	...
	if ( IsInPlayerSquad() )
	{
		...
		if ( gpGlobals->curtime - lastTime > 2.0 || !hCurrent || !hCurrent->IsInPlayerSquad() ) // hCurrent will be NULL after level change
		{
			...
			if ( pPlayer )
			{
				...
				for ( CAI_BaseNPC *pAllyNpc = m_pSquad->GetFirstMember(&iter); pAllyNpc; pAllyNpc = m_pSquad->GetNextMember(&iter) )
				{
					if ( pAllyNpc->IsCommandable() && dynamic_cast<CNPC_RANTOS *>(pAllyNpc) )
					{
						int i = candidates.AddToTail();
						candidates[i].pMember = (CNPC_RANTOS *)(pAllyNpc);
		...
		if ( hCurrent != NULL )
		{
			Assert( dynamic_cast<CNPC_RANTOS *>(hCurrent.Get()) && hCurrent->IsInPlayerSquad() );
			return hCurrent;
		}
	}
	return NULL;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::SetSquad( CAI_Squad *pSquad )
{
	...
	if( IsInPlayerSquad() && !bWasInPlayerSquad )
	{
		...
		if ( npc_ranto_s_insignia.GetBool() )
			AddInsignia();
	}
...
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	...
	else if (interactionType ==	g_interactionScannerInspectBegin)
	{
		...
		{
			m_nInspectActivity = ACT_RANTOS_STARTLED;
		}
		else
		{
			// Otherwise we're blinded by the flash
			m_nInspectActivity = ACT_RANTOS_BLINDED;
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::FValidateHintType( CAI_Hint *pHint )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::ShouldHealTarget( CBaseEntity *pTarget, bool bActiveUse )
{
			...
			)
	 	{
			if ( pTarget->m_iHealth > 0 )
			{
	 			if ( bActiveUse )
				{
					...
					float timeRecharge = arsenio_rantos_heal_player_delay.GetFloat();
					float maximumHealAmount = arsenio_rantos_heal_player.GetFloat();
					...
					if ( healAmt < arsenio_rantos_heal_player_min_forced.GetFloat() )
						return false;
				...
				if ( bReadyToHeal )
				{
					...
					if ( bTargetIsPlayer )
						requiredHealth = pTarget->GetMaxHealth() - arsenio_rantos_heal_player.GetFloat();
					else
						requiredHealth = pTarget->GetMaxHealth() * arsenio_rantos_heal_player_min_pct.GetFloat();
	...
	if ( IsAmmoResupplier() && bTargetIsPlayer )
	{
		if ( m_flPlayerGiveAmmoTime <= gpGlobals->curtime )
		{
			...
			if ( iAmmoType == -1 )
			{
				DevMsg("ERROR: Rantos attempting to give unknown ammo type (%s)\n", STRING(m_iszAmmoSupply) );
			}
			else
			{
...
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::ShouldHealTossTarget( CBaseEntity *pTarget, bool bActiveUse )
{
	Disposition_t disposition;
...
void CNPC_RANTOS::Heal()
{
	...
	m_flTimeNextHealStare = gpGlobals->curtime + arsenio_rantos_stare_heal_time.GetFloat();
	...
	if ( IsMedic() )
	{
		...
		if ( pTarget->IsPlayer() )
		{
			...
			timeRecharge 		= arsenio_rantos_heal_player_delay.GetFloat();
			maximumHealAmount 	= arsenio_rantos_heal_player.GetFloat();
			m_flPlayerHealTime 	= gpGlobals->curtime + timeRecharge;
		}
		else
		{
			timeFullHeal 		= m_flAllyHealTime;
			timeRecharge 		= arsenio_rantos_heal_ally_delay.GetFloat();
			maximumHealAmount 	= arsenio_rantos_heal_ally.GetFloat();
		...
		if ( healAmt > 0 )
		{
			if ( pTarget->IsPlayer() && npc_ranto_s_medic_emit_sound.GetBool() )
			{
	...
	if ( IsAmmoResupplier() )
	{
		...
		if ( pTarget->IsPlayer() )
		{
			...
			m_flPlayerGiveAmmoTime = gpGlobals->curtime + arsenio_rantos_giveammo_player_delay.GetFloat();
		}
	}
}
...
//-----------------------------------------------------------------------------
void	CNPC_RANTOS::TossHealthKit(CBaseCombatCharacter *pThrowAt, const Vector &offset)
{
	Assert( pThrowAt );
	...
	if (npc_ranto_s_medic_throw_style.GetInt() == 0)
	{
	...
	{
		Warning("Rantos tried to heal but could not spawn item_healthkit!\n");
	}
...
//-----------------------------------------------------------------------------
void	CNPC_RANTOS::InputForceHealthKitToss( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::ShouldLookForHealthItem()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::InputStartPatrolling( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::InputStopPatrolling( inputdata_t &inputdata )
{
	m_bShouldPatrol = false;
}
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_RANTOS::OnGivenWeapon( CBaseCombatWeapon *pNewWeapon )
{
	FixupMattWeapon();
}
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_RANTOS::InputSetCommandable( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_RANTOS::InputSetMedicOn( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_RANTOS::InputSetMedicOff( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_RANTOS::InputSetAmmoResupplierOn( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_RANTOS::InputSetAmmoResupplierOff( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_RANTOS::InputSpeakIdleResponse( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_RANTOS::DeathSound( const CTakeDamageInfo &info )
{
	// Sentences don't play on dead NPCs
	SentenceStop();

	EmitSound( "NPC_Rantos.Die" );
}

//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_RANTOS::FearSound( void )
{
}
...
//-----------------------------------------------------------------------------
bool CNPC_RANTOS::UseSemaphore( void )
{
...
AI_BEGIN_CUSTOM_NPC( npc_ranto_s, CNPC_RANTOS )
	...
	DECLARE_ACTIVITY( ACT_RANTOS_HANDSUP )
	DECLARE_ACTIVITY( ACT_RANTOS_BLINDED )
	DECLARE_ACTIVITY( ACT_RANTOS_SHOWARMBAND )
	DECLARE_ACTIVITY( ACT_RANTOS_HEAL )
	DECLARE_ACTIVITY( ACT_RANTOS_STARTLED )
	...
	//Events
	DECLARE_ANIMEVENT( AE_RANTOS_GET_PACKAGE )
	DECLARE_ANIMEVENT( AE_RANTOS_HEAL )
...
//
//==================================================================================================================
CHandle<CRantosResponseSystem>	g_pRantosResponseSystem = NULL;

CRantosResponseSystem	*GetRantosResponse()
{
	return g_pRantosResponseSystem;
}

char *RantosResponseConcepts[MAX_CITIZEN_RESPONSES] = 
{
...
LINK_ENTITY_TO_CLASS( ai_rantos_response_system, CRantosResponseSystem );

BEGIN_DATADESC( CRantosResponseSystem )
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRantosResponseSystem::Spawn()
{
	if ( g_pRantosResponseSystem )
	{
		Warning("Multiple citizen response systems in level.\n");
		UTIL_Remove( this );
		return;
	}
	g_pRantosResponseSystem = this;

	// Invisible, non solid.
	AddSolidFlags( FSOLID_NOT_SOLID );
	AddEffects( EF_NODRAW );
	SetThink( &CRantosResponseSystem::ResponseThink );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRantosResponseSystem::OnRestore()
{
	BaseClass::OnRestore();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRantosResponseSystem::AddResponseTrigger( rantosresponses_t	iTrigger )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRantosResponseSystem::InputResponseVitalNPC( inputdata_t &inputdata )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRantosResponseSystem::ResponseThink()
{
		...
		for ( int i = 0; i < MAX_CITIZEN_RESPONSES; i++ )
		{
			if ( m_flResponseAddedTime[i] )
			{
				...
				else if ( m_flNextResponseTime < gpGlobals->curtime )
				{
					...
					CBaseEntity *pNearestRantos = NULL;
					CBaseEntity *pRantos = NULL;
					CBasePlayer *pPlayer = UTIL_GetLocalPlayer();
					while ( (pRantos = gEntList.FindEntityByClassname( pRantos, "npc_ranto_s" ) ) != NULL)
					{
						float flDistToPlayer = (pPlayer->WorldSpaceCenter() - pRantos->WorldSpaceCenter()).LengthSqr();
						if ( flDistToPlayer < flNearestDist )
						{
							flNearestDist = flDistToPlayer;
							pNearestRantos = pRantos;
						}
					}

					// Found one?
					if ( pNearestRantos && ((CNPC_RANTOS*)pNearestRantos)->RespondedTo( RantosResponseConcepts[i], false, false ) )
					{
						m_flResponseAddedTime[i] = 0;
...
void CNPC_RANTOS::RemoveInsignia()
{
...
//-----------------------------------------------------------------------------
int CNPC_RANTOS::DrawDebugTextOverlays( void ) 
{
	...
	if (m_debugOverlays & OVERLAY_TEXT_BIT) 
	{
		...
		Q_snprintf(tempstr,sizeof(tempstr),"Expression type: %s", szExpressionTypeRantoss[m_ExpressionTypeRantos]);

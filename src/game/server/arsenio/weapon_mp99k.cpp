...
// GLITCHY: Look, I'll think of an original mechanic eventually.
class CWeaponMP99K : public CHLMachineGun
{
	DECLARE_DATADESC();
public:
...
void CWeaponMP99K::BurstAttack( int burstSize, float cycleRate, int spentAmmoModifier, float spreadComponentDivisor )
{
	...
	if (!GetOwner())
	{
		Msg( "Error: MP99K has NULL owner!" );
		return;
	}
	...
	while (m_flNextPrimaryAttack <= gpGlobals->curtime)
	{
		...
		if (m_iBurstSize >= burstSize) {
			...
			m_flNextSecondaryAttack = m_flNextPrimaryAttack + cycleRate; // MP99K shares primary attack between primary and secondary
		}
		else {
			m_flNextPrimaryAttack = m_flNextPrimaryAttack + fireRate;
			m_flNextSecondaryAttack = m_flNextPrimaryAttack + fireRate; // MP99K shares primary attack between primary and secondary
		}

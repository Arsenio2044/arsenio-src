...
ConVar	sk_bot_enemy_health( "sk_bot_enemy_health","100");
ConVar	sk_bot_enemy_kick( "sk_bot_enemy_kick","35");

ConVar sk_bot_enemy_guard_health( "sk_bot_enemy_guard_health", "0");
ConVar sk_bot_enemy_guard_kick( "sk_bot_enemy_guard_kick", "0");
 
// Whether or not the bot_enemy guard should spawn health on death
ConVar bot_enemy_guard_spawn_health( "bot_enemy_guard_spawn_health", "1" );

extern ConVar sk_plr_dmg_buckshot;	
extern ConVar sk_plr_num_shotgun_pellets;

//Whether or not the bot_enemy should spawn health on death
ConVar	bot_enemypawn_health( "bot_enemypawn_health", "1" );

ConVar  add_fuck_prob( "add_yourmom_prob", "0", 0,
	"Every bot_enemy soldier has this chance to spawn a hunter" );

LINK_ENTITY_TO_CLASS( bot_enemy, CBotEnemy );
...
void CBotEnemy::Spawn( void )
{
	...
	{
		// Stronger, tougher.
		SetHealth( sk_bot_enemy_guard_health.GetFloat() );
		SetMaxHealth( sk_bot_enemy_guard_health.GetFloat() );
		SetKickDamage( sk_bot_enemy_guard_kick.GetFloat() );
	}
	else
	{
		SetHealth( sk_bot_enemy_health.GetFloat() );
		SetMaxHealth( sk_bot_enemy_health.GetFloat() );
		SetKickDamage( sk_bot_enemy_kick.GetFloat() );
	}
	...
	if ( add_fuck_prob.GetFloat() > 0 )
	{
		if (RandomFloat() <= add_fuck_prob.GetFloat())
		{
			...
			pHunter->Precache(); // should already be precached from the bot_enemy soldier's precache
...
void CBotEnemy::DeathSound( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
void CBotEnemy::ClearAttackConditions( )
{
...
void CBotEnemy::PrescheduleThink( void )
{
...
//-----------------------------------------------------------------------------
void CBotEnemy::BuildScheduleTestBits( void )
{
...
// Output :
//-----------------------------------------------------------------------------
int CBotEnemy::SelectSchedule ( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CBotEnemy::GetHitgroupDamageMultiplier( int iHitGroup, const CTakeDamageInfo &info )
{
	switch( iHitGroup )
	{
	case HITGROUP_HEAD:
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBotEnemy::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
...
void CBotEnemy::OnChangeActivity( Activity eNewActivity )
{
...
//-----------------------------------------------------------------------------
void CBotEnemy::Event_Killed( const CTakeDamageInfo &info )
{
	// Don't bother if we've been told not to, or the player has a megaphyscannon
	if ( bot_enemypawn_health.GetBool() == false || PlayerHasMegaPhysCannon() )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBotEnemy::IsLightDamage( const CTakeDamageInfo &info )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBotEnemy::IsHeavyDamage( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
Activity CBotEnemy::NPC_TranslateActivity( Activity eNewActivity )
{

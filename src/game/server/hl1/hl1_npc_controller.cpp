...
ConVar arsenio_controller_health ( "arsenio_controller_health", "60" );
ConVar arsenio_controller_dmgzap ( "arsenio_controller_dmgzap", "15" );
ConVar arsenio_controller_speedball ( "arsenio_controller_speedball", "650" );
ConVar arsenio_controller_dmgball ( "arsenio_controller_dmgball", "3" );
...
public:
	...
	Class_T	Classify ( void ) { return	CLASS_ZOMBIE; } // TUX: Until I create a class for them.
...
void CNPC_Controller::Spawn()
{
	...
	m_iHealth =			arsenio_controller_health.GetFloat();
...
void CNPC_Controller::Event_Killed( const CTakeDamageInfo &info )
{
	if( ShouldGib(info) )
	{
		//remove the balls TUX: funny haha balls lmao
		if (m_pBall[0])
		{
	...
	if (m_flShootEnd > gpGlobals->curtime)
	{
		...
		while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->curtime)
		{
			...
			if (GetEnemy() != NULL)
			{
				...
				vecDir = Intersect( vecSrc, GetEnemy()->BodyTarget( GetAbsOrigin() ), m_vecEstVelocity, arsenio_controller_speedball.GetFloat() );
			
				float delta = 0.03490; // +-2 degree
				vecDir = vecDir + Vector( random->RandomFloat( -delta, delta ), random->RandomFloat( -delta, delta ), random->RandomFloat( -delta, delta ) ) * arsenio_controller_speedball.GetFloat();
...
void CNPC_ControllerHeadBall::HuntThink( void  )
{
	...
	if ((GetEnemy()->WorldSpaceCenter() - GetAbsOrigin()).Length() < 64)
	{
		...
		if (pEntity != NULL && pEntity->m_takedamage == DAMAGE_YES)
		{
			...
			VectorNormalize( dir );
			CTakeDamageInfo info( this, this, arsenio_controller_dmgball.GetFloat(), DMG_SHOCK );
...
void CNPC_ControllerZapBall::ExplodeTouch( CBaseEntity *pOther )
{
	if (m_takedamage = DAMAGE_YES )
	{
		...
		if (m_hOwner != NULL)
		{
			CTakeDamageInfo info( this, m_hOwner, arsenio_controller_dmgball.GetFloat(), DMG_ENERGYBEAM );
			CalculateMeleeDamageForce( &info, vecAttackDir, tr.endpos );
			pOther->DispatchTraceAttack( info, vecAttackDir, &tr );
		}
		else
		{
			CTakeDamageInfo info( this, this, arsenio_controller_dmgball.GetFloat(), DMG_ENERGYBEAM );
			CalculateMeleeDamageForce( &info, vecAttackDir, tr.endpos );
			pOther->DispatchTraceAttack( info, vecAttackDir, &tr );
		}

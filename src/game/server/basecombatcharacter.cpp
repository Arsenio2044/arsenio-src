...
// Mobility - smg and ar2 get tighter spread if crouching
ConVar sk_duck_acc_bonus( "sk_duck_acc_bonus", "0.5" );
...
void                
CBaseCombatCharacter::LandOnMe( CBaseEntity *guy ) {
	CTakeDamageInfo dmginfo( guy, guy, 180.0, DMG_DISSOLVE );
...
Vector CBaseCombatCharacter::GetAttackSpread( CBaseCombatWeapon *pWeapon, CBaseEntity *pTarget )
{
	if (pWeapon)
	{
		Vector vecCone = pWeapon->GetBulletSpread( GetCurrentWeaponProficiency() );
		if (IsPlayer())
		{
			CBasePlayer* pPlayer = ToBasePlayer( this );
			if (pPlayer->m_Local.m_bDucked &&
				!pPlayer->m_bIsPowerSliding && 
				pWeapon->CrouchingImprovesAccuracy())
			{
				vecCone *= sk_duck_acc_bonus.GetFloat();
			}
		}

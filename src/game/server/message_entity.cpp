...
public:
	...
	void    InputDisplayTime( inputdata_t &inputdata );
...
BEGIN_DATADESC( CMessageEntity )
	...
	DEFINE_INPUTFUNC( FIELD_INTEGER,    "ShowTime", InputDisplayTime ),
...
void CMessageEntity::InputDisplayTime( inputdata_t &inputdata ) 
{
	Msg( "Received Time: " );
	int deciSeconds = inputdata.value.Int();
	...
	char buffer[16];
	Q_snprintf( buffer, 16, "%d:%d:%d.%d", hour, min, sec, dsec );

	Msg("%s\n", buffer );

	m_messageText = MAKE_STRING( buffer );
	m_bEnabled = true;
}

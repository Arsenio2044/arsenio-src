...
// Experimental alias. Want the bomb to show up if we search by classname npc_*
LINK_ENTITY_TO_CLASS( npc_chopperbomb, CGrenadeHelicopter );
...
void CGrenadeHelicopter::Spawn( void )
{
	...
	if (IsHL2Map())
		...
		// Episodic behaviour is no damage
		m_takedamage = DAMAGE_NO;
...
int CGrenadeHelicopter::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if (info.GetDamageType() & DMG_BLAST)
	{
		// except from a homing missile (airboat rockets)
		CBaseEntity* inflictor = info.GetInflictor();
		if (!inflictor || !inflictor->ClassMatches("homing_missile"))
			return 0;
	}

...
void CBreakableProp::Spawn()
{
	...
	// MobMod - seeing some weird fade effects on some props, try disabling fade
...
void CDynamicProp::Spawn( )
{
	...
	// MobMod HACK for Black Mesa East elevator

	/* Disabled - doesn't work anyway
	if ( Q_strcmp( STRING( gpGlobals->mapname ), "d1_eli_01") == 0  &&
		( Q_strcmp( STRING( GetEntityName() ), "prop_elevatordoor_top_1" ) == 0 || 
		  Q_strcmp( STRING( GetEntityName() ), "prop_elevatordoor_bottom_1" ) == 0 ||
		  Q_strstr( STRING( GetModelName() ), "freightelevator.mdl" ) >= 0 ) )
	{
		SetSolid( SOLID_NONE );
		AddSolidFlags( FSOLID_NOT_SOLID );
	}*/
...
void CPropDoorRotating::Spawn()
{
	...
	// HACKHACK MobMod - one scene in d2_prison_06 sometimes gets stuck,
	// alyx and eli just sit there, player can't progress. Hack this one door
	// to just spawn open and let us skip the whole scene. 
	if (Q_strcmp( STRING( gpGlobals->mapname ), "d2_prison_06" ) == 0 &&
		Q_strcmp( STRING( GetEntityName() ), "door_controlroom_1" ) == 0)
	{

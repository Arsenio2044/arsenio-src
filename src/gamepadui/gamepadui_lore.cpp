...
const int MAX_LORE_TABS = 8;

#define GAMEPADUI_LORE_FILE GAMEPADUI_RESOURCE_FOLDER "lore.res"
...
class GamepadUILorePanel : public GamepadUIFrame
{
    DECLARE_CLASS_SIMPLE( GamepadUILorePanel, GamepadUIFrame );

public:
    GamepadUILorePanel( vgui::Panel *pParent, const char* pPanelName );
    ~GamepadUILorePanel();
    ...
    void LoadOptionTabs( const char* pszLoreFile );
    ...
    static GamepadUILorePanel *GetInstance()
    {
        return s_pLorePanel;
    }
...
private:
    ...
    GamepadUITab m_Tabs[ MAX_LORE_TABS ];
    int m_nTabCount = 0;
    ...
    static GamepadUILorePanel *s_pLorePanel;

};

GamepadUILorePanel* GamepadUILorePanel::s_pLorePanel = NULL;

ConVar gamepadui_last_lore_tab( "gamepadui_last_lore_tab", "0", FCVAR_ARCHIVE );
...
public:
    ...
    void OnKeyCodePressed( vgui::KeyCode code )
    {
        ...
        case STEAMCONTROLLER_DPAD_LEFT:
            ...
            if ( --m_nSelectedItem < 0 )
                m_nSelectedItem = Max( 0, m_Lore.Count() - 1 );
            if ( m_bInstantApply )
                UpdateConVar();
            break;
...
#endif
            if ( m_Lore.Count() )
                m_nSelectedItem = ( m_nSelectedItem + 1 ) % m_Lore.Count();
            if ( m_bInstantApply )
                UpdateConVar();
            break;
    ...
    void UpdateConVar() OVERRIDE
    {
        ...
        {
            if ( m_bSignOnly )
                m_cvar.SetValue( abs( m_cvar.GetFloat() ) * m_Lore[m_nSelectedItem].nValue );
            else
                m_cvar.SetValue( m_Lore[ m_nSelectedItem ].nValue );
        }
    }
    ...
    {
        return m_cvar.IsValid() && GetCvarValue() != m_Lore[m_nSelectedItem].nValue;
    }
    ...
    virtual void Paint()
    {
        ...
        if ( m_nSelectedItem >= m_Lore.Count() )
            return;

        GamepadUIString& strOption = m_Lore[ m_nSelectedItem ].strOptionText;
        ButtonState state = GetCurrentButtonState();
    ...
    void SetToDefault() OVERRIDE
    {
        if ( m_cvar.IsValid() )
        {
            ...
            for ( int i = 0; i < m_Lore.Count(); i++)
            {
                if ( m_Lore[ i ].nValue == nCurrentValue )
                    m_nSelectedItem = i;
            }
        }
    }
    ...
    {
        if ( nIndex < 0 || nIndex >= m_Lore.Count() )
            return NULL;
        return &m_Lore[ nIndex ];
    }

    bool IsConVarEnabled() const OVERRIDE
    {
        return !!m_Lore[m_nSelectedItem].nValue;
    }
...
private:
    ...
    CUtlVector< GamepadUIOption > m_Lore;
...
void FlushPendingResolution()
{
    ...
    GamepadUIWheelyWheel *pResButton = pLore->GetResolutionButton();
    if ( !pResButton )
        return;
    ...
    : BaseClass( pParent, pPanelName )
{
    ...
    GetFrameTitle() = GamepadUIString( "#GameUI_Lore" );
...
GamepadUILorePanel::~GamepadUILorePanel()
{
    s_pLorePanel = NULL;
}

void GamepadUILorePanel::OnThink()
{
    BaseClass::OnThink();
...
void GamepadUILorePanel::UpdateGradients()
{
...
void GamepadUILorePanel::LayoutCurrentTab()
{
    ...
    for ( GamepadUIButton *pButton : m_Tabs[ nActiveTab ].pButtons )
    {
        ...
        else
        {
            ...
            if ( buttonY < y )
                fade = RemapValClamped( y - buttonY, m_flLoreFade - pButton->GetTall(), 0, 0, 255 );
            else if ( buttonY > ( nParentH - m_flFooterButtonsOffsetY - m_nFooterButtonHeight - m_flLoreFade ) )
                fade = RemapValClamped( ( nParentH - m_flFooterButtonsOffsetY - m_nFooterButtonHeight ) - ( buttonY + pButton->GetTall() ), 0, m_flLoreFade, 0, 255 );
...
void GamepadUILorePanel::OnMouseWheeled( int delta )
{
    m_Tabs[ GetActiveTab() ].ScrollState.OnMouseWheeled( delta * 100.0f, GamepadUI::GetInstance().GetTime() );
}

void GamepadUILorePanel::OnCommand( char const* pCommand )
{
...
int GamepadUILorePanel::GetActiveTab()
{
    int nUserTab = gamepadui_last_lore_tab.GetInt();

    int nActiveTab = Clamp( nUserTab, 0, Max( 0, m_nTabCount - 1 ) );
    if ( nUserTab != nActiveTab )
        gamepadui_last_lore_tab.SetValue( nActiveTab );
    return nActiveTab;
}
...
void GamepadUILorePanel::UpdateResolutions()
{
    if ( !m_pResolutionButton )
        return;

    m_pResolutionButton->ClearLore();
...
// Mainly from GameUI
void GamepadUILorePanel::FillInBindings()
{
    for ( int i = 0; i < m_nTabCount; i++ )
...
void GamepadUILorePanel::ApplyKeyBindings()
{
...
void GamepadUILorePanel::OnKeyBound( const char *pKey )
{
    for ( int i = 0; i < m_nTabCount; i++ )
    {
...
void GamepadUILorePanel::OnKeyCodePressed( vgui::KeyCode code )
{
    ButtonCode_t buttonCode = GetBaseButtonCode( code );
...
void GamepadUILorePanel::OnGamepadUIButtonNavigatedTo( vgui::VPANEL button )
{
...
void GamepadUILorePanel::SetActiveTab( int nTab )
{
    nTab = Clamp( nTab, 0, Max( m_nTabCount - 1, 0 ) );

    gamepadui_last_lore_tab.SetValue( nTab );
    int nActiveTab = GetActiveTab();
    for ( int i = 0; i < m_nTabCount; i++ )
...
void GamepadUILorePanel::LoadOptionTabs( const char *pszLoreFile )
{
    KeyValues* pDataFile = new KeyValues( "Lore" );
    if ( pDataFile->LoadFromFile( g_pFullFileSystem, pszLoreFile ) )
    {
        ...
        {
            {
                ...
                if ( m_nTabCount == gamepadui_last_lore_tab.GetInt() )
            ...
            if ( pTabItems )
            {
                for ( KeyValues* pItemData = pTabItems->GetFirstSubKey(); pItemData != NULL; pItemData = pItemData->GetNextKey() )
                {
                    ...
                    else if ( !V_strcmp( pItemType, "wheelywheel" ) )
                    {
                        ...
                        const char *pszLoreFrom = pItemData->GetString( "lore_from" );
                        KeyValues *pLore = pItemData->FindKey( "lore" );
                        if ( pLore )
                        {
                            for ( KeyValues* pOptionData = pLore->GetFirstSubKey(); pOptionData != NULL; pOptionData = pOptionData->GetNextKey() )
                            {
                                GamepadUIOption option;
                        ...
                        else if ( pszLoreFrom && *pszLoreFrom )
                        {
                            ...
                            else if ( !V_strcmp( pszLoreFrom, "displaymode" ) )
                            {
                                int i = 0;
                            ...
                            else if ( !V_strcmp( pszLoreFrom, "resolutions" ) )
                            {
...
CON_COMMAND( gamepadui_openloredialog, "" )
{

    new GamepadUILorePanel( GamepadUI::GetInstance().GetBasePanel(), "" );
}

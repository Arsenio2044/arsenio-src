...
class GamepadUIMobModePanel : public GamepadUIFrame
{
    DECLARE_CLASS_SIMPLE( GamepadUIMobModePanel, GamepadUIFrame );

public:
    GamepadUIMobModePanel( vgui::Panel *pParent, const char* pPanelName );
...
public:
    ...
    {
        if ( s_pLastMobModeButton == this )
            s_pLastMobModeButton = NULL;
    }
    ...
    void NavigateTo() OVERRIDE
    {
        ...
        s_pLastMobModeButton = this;
    }

    static GamepadUIChapterButton* GetLastMobModeButton() { return s_pLastMobModeButton; }

private:
    GamepadUIImage m_Image;

    static GamepadUIChapterButton *s_pLastMobModeButton;
};

GamepadUIChapterButton* GamepadUIChapterButton::s_pLastMobModeButton = NULL;
...
static int FindChapters( chapter_t *pChapters )
{
    ...
    const char *fileName = "mob/chapter*.mob";
    fileName = g_pFullFileSystem->FindFirst( fileName, &findHandle );
    while ( fileName && chapterIndex < MAX_CHAPTERS )
    {
        if ( fileName[0] )
        {
            // Only load chapter configs from the current mod's mob dir
            // or else chapters appear that we don't want!
            Q_snprintf( szFullFileName, sizeof( szFullFileName ), "mob/%s", fileName );
...
GamepadUIMobModePanel::GamepadUIMobModePanel( vgui::Panel *pParent, const char* PanelName ) : BaseClass( pParent, PanelName )
{
    vgui::HScheme hScheme = vgui::scheme()->LoadSchemeFromFile( GAMEPADUI_DEFAULT_PANEL_SCHEME, "SchemePanel" );
    SetScheme( hScheme );

    GetFrameTitle() = GamepadUIString( "#GameUI_MobMode" );
    ...
    for ( int i = 0; i < nChapterCount; i++ )
    {
        ...
        // strip the extension
        char *ext = V_stristr( chapterID, ".mob" );
        if ( ext )
        {
            *ext = 0;
        }
...
void GamepadUIMobModePanel::UpdateGradients()
{
...
void GamepadUIMobModePanel::OnThink()
{
    BaseClass::OnThink();
...
void GamepadUIMobModePanel::OnGamepadUIButtonNavigatedTo( vgui::VPANEL button )
{
...
void GamepadUIMobModePanel::LayoutChapterButtons()
{
    int nParentW, nParentH;
	GetParent()->GetSize( nParentW, nParentH );
...
void GamepadUIMobModePanel::OnCommand( char const* pCommand )
{
    ...
    {
        GamepadUIChapterButton *pPanel = GamepadUIChapterButton::GetLastMobModeButton();
        if ( pPanel )
        {
...
void GamepadUIMobModePanel::OnMouseWheeled( int nDelta )
{
...
void GamepadUIMobModePanel::StartGame( int nChapter )
{
    MapCommand_t cmd;
    cmd.szCommand[0] = 0;

...
void CLoadingTipPanel::SetupTips( void )
{
	...
	for ( KeyValues *pKey = pKV->FindKey( "SurvivorTips" )->GetFirstSubKey(); pKey; pKey = pKey->GetNextKey() )
	{
		...
		sTipInfo info;
		V_snprintf( info.szTipTitle, MAX_TIP_LENGTH, "#%s_NAME"  );
		V_snprintf( info.szTipString, MAX_TIP_LENGTH, "#%s_DESC"  );
		V_snprintf( info.szTipImage, MAX_TIP_LENGTH, "achievements/%s" );
		m_Tips.AddToTail( info );
...
void PrecacheLoadingTipIcons()
{
		...
		CFmtStr imageString( "vgui/achievements/%s" );

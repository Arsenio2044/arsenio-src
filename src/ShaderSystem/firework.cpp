...
//
// Purpose: Implement the FireWork rendering system.
//
//
//=============================================================================//
#include "convar.h"
...
#include "firework/ifireworkrenderables.h"
#include "firework.h"
...
// Let's identify the DLL
static CFireWorkRenderer g_FireWorkRenderer;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CFireWorkRenderer, IFireWorkRenderer, FIREWORK_INTERFACE_VERSION, g_FireWorkRenderer);

bool CFireWorkRenderer::Connect(CreateInterfaceFn factory)
{
...
void CFireWorkRenderer::Disconnect()
{
	return BaseClass::Disconnect();
}

void* CFireWorkRenderer::QueryInterface(const char* pInterfaceName)
{
	// Loading the firework DLL mounts *all* interfaces
	CreateInterfaceFn factory = Sys_GetFactoryThis();
	return factory(pInterfaceName, NULL);
}

InitReturnVal_t CFireWorkRenderer::Init()
{
	return INIT_OK;
...
CON_COMMAND(FireWork, "List of arguments: version, help")
{
	if (args.ArgC() > 1)
	{
		...
		{
			Msg("\nFireWork Rendering ");
			ConColorMsg(Color(0, 148, 255, 255), "Created by Tuxxego.");
			Msg(" Ported to source by GLITCH SOFTWARE.\n");
		}
	}
	else
	{
...
void CFireWorkRenderer::Shutdown()
{
	return BaseClass::Shutdown();
}

void CFireWorkRenderer::StartFrame()
{
	// prepare frame here or something idk
}

void CFireWorkRenderer::Render(const CViewSetup& view, int nClearFlags, int whatToDraw)
{
	Push3DView(view);
...
void CFireWorkRenderer::EndFrame()
{
	m_Renderables.Purge();
}

void CFireWorkRenderer::DrawRenderables()
{
...
void CFireWorkRenderer::PushRenderable(IFireWorkRenderer_Renderable* pRenderable)
{
...
void CFireWorkRenderer::BuildViewMatrix(const CViewSetup& view, VMatrix& matview)
{
...
void CFireWorkRenderer::BuildProjMatrix(const CViewSetup& view, VMatrix& matproj)
{
...
void CFireWorkRenderer::Push3DView(const CViewSetup& view)
{
...
void CFireWorkRenderer::PopView()
{
	CMatRenderContextPtr pRenderContext(materials);
